...
public sealed class StorageSystem : SharedStorageSystem
{
    ...
    public void PickupAnimation(EntityUid item, EntityCoordinates initialCoords, EntityCoordinates finalCoords, Angle initialAngle)
    {
        ...
        if (finalCoords.InRange(EntityManager, XformSystem, initialCoords, 0.1f) ||
            !Exists(initialCoords.EntityId) || !Exists(finalCoords.EntityId))
        {
            return;
        }

        var finalMapPos = finalCoords.ToMapPos(EntityManager, XformSystem);
        var finalPos = XformSystem.GetInvWorldMatrix(initialCoords.EntityId).Transform(finalMapPos);
