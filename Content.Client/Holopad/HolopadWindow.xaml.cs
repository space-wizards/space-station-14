...
public sealed partial class HolopadWindow : FancyWindow
{
    ...
    public void Refresh()
    {
        ...
        var query = _entManager.AllEntityQueryEnumerator<HolopadComponent, TelephoneComponent, MetaDataComponent>();

        while (query.MoveNext(out var uid, out var holopadComponent, out var telephoneComponent, out var metaDataComponent))
        {
            // Filter holopads
            if (_owner == uid)
                continue;

            if (telephoneComponent.UnlistedNumber)
                continue;

            if (!_telephoneSystem.IsSourceInRangeOfReceiver((_owner.Value, telephone), (uid, telephoneComponent)))
                continue;

            // Get holopad name
            var name = metaDataComponent.EntityName;

            if (_entManager.TryGetComponent<LabelComponent>(uid, out var label) && !string.IsNullOrEmpty(label.CurrentLabel))
                name = label.CurrentLabel;

            holopadList.Add((_entManager.GetNetEntity(uid), name));
        }
