...
public partial class BaseShuttleControl : MapGridControl
{
    ...
    private readonly List<Vector2> _edges = new();
    ...
    protected void DrawGrid(DrawingHandleScreen handle, Matrix3 matrix, Entity<MapGridComponent> grid, Color color, float alpha = 0.01f)
    {
        ...
        for (var i = 0; i < _gridTileList.Count; i++)
        {
            ...
            if (DrawInterior)
            {
                ...
                _tileTris.Add(scaledBL);
                _tileTris.Add(scaledBR);
                _tileTris.Add(scaledTL);

                _tileTris.Add(scaledBR);
                _tileTris.Add(scaledTL);
                _tileTris.Add(scaledTR);
            }
            ...
            foreach (var (dir, _) in _neighborDirections)
            {
                ...
                _edges.Add(actualStart);
                _edges.Add(actualEnd);
            }

            if (_tileTris.Count > TileBatchCount)
            {
                FlushGridDraw(handle, color, DrawInterior, alpha);
            }
        }
    ...
    private void FlushGridDraw(DrawingHandleScreen handle, Color color, bool drawInterior, float alpha)
    {
        if (drawInterior)
        {
            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList, _tileTris, color.WithAlpha(alpha));
        }

        handle.DrawPrimitives(DrawPrimitiveTopology.LineList, _edges, color);
        _tileTris.Clear();
        _edges.Clear();
    }
