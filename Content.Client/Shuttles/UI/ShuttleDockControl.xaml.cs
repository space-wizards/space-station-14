        ...
        foreach (var grid in _grids)
        {
            ...
            foreach (var dock in gridDocks)
            {
                ...
                if (viewedDockPos != null && dock.Coordinates.NetEntity != gridNent)
                {
                    ...
                    if (range.LengthSquared() < SharedDockingSystem.DockingHiglightRange * MinimapScale * SharedDockingSystem.DockingHiglightRange * MinimapScale)
                    {
                        if (_viewedState?.GridDockedWith == null)
                        {
                            ...
                            var distanceSquared = (mapOne.Position - mapTwo.Position).LengthSquared();

                            var inAlignment = _dockSystem.InAlignment(mapOne, rotA, mapTwo, rotB);
                            var canDock = distanceSquared < SharedDockingSystem.DockRange * SharedDockingSystem.DockRange && inAlignment;
