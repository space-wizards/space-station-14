...
public sealed partial class SurveillanceCameraMonitorWindow : DefaultWindow
{
    ...
    /// </summary>
    public event Action<DeviceAddress, DeviceAddress?>? CameraSelected;

    public event Action<DeviceAddress>? SubnetOpened;
    ...
    private readonly Dictionary<DeviceAddress, int> _subnetMap = new();
    ...
    public SurveillanceCameraMonitorWindow()
    {
        ...
        {
            // piss
            SubnetOpened?.Invoke((DeviceAddress) args.Button.GetItemMetadata(args.Id)!);
        };
    ...
    public void UpdateState(IEye? eye, Dictionary<DeviceAddress, DeviceFrequency> subnets, DeviceAddress activeAddress, DeviceAddress activeSubnet, Dictionary<DeviceAddress, string> cameras)
    {
        ...
        if (!activeSubnet.IsValid())
        {
            SubnetOpened!(subnets.Keys.First());
            return;
        }
        ...
        if (SubnetSelector.ItemCount != subnets.Count)
        {
            ...
            foreach (var (subnet, frequency) in subnets)
            {
                var id = AddSubnet(subnet, frequency);
    ...
    private int AddSubnet(DeviceAddress subnet, DeviceFrequency freq)
    {
        var name = freq.FrequencyId.ToString();

        foreach (var freqProto in _prototypeManager.EnumeratePrototypes<DeviceFrequencyPrototype>())
        {
            if (freqProto.Frequency == freq
                && freqProto.Name != null)
                name = Loc.GetString(freqProto.Name);
        }
