...
public sealed class RCDMenuBoundUserInterface : BoundUserInterface
{
    ...
    protected override void Open()
    {
        ...
        var dict = new Dictionary<string, (string Tooltip, SpriteSpecifier Sprite)>
        {
        ...
        var models = rcd.AvailablePrototypes.Select(x => _prototypeManager.Index(x))
                        .GroupBy(x => x.Category)
                        .Where(x=> dict.ContainsKey(x.Key))
                        .Select(x =>
                        {
                            var nested = x.Select(proto => new RadialMenuButtonModel(() =>
                            {
                            ...
                            var tuple = dict[x.Key];
                            return new RadialMenuButtonModel(nested)
                            {
                                Sprite = tuple.Sprite,
