...
public sealed partial class ReagentStorageControl : ScrollContainer
{
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        // TODO: Unhardcode that fucking name
        if (!_solution.TryGetSolution(_owner.Value, "lathe", out _, out var solution))
        {
            Visible = false;
            return;
        }
        ...
        {
            if (!solution.ContainsReagent(id, null))
                remSolutions.Add(id);
        }
        ...
        foreach (var display in children2)
        {
            ...
            var vol = solution.GetTotalPrototypeQuantity(reagent);

            if (vol == FixedPoint2.Zero)
                continue;

            display.UpdateVolume(vol, solution);
        }

        foreach (var reagent in newSolutions)
        {
            var vol = solution.GetTotalPrototypeQuantity(reagent);
            ReagentList.AddChild(new ReagentDisplay(_owner.Value, reagent, vol, solution));
        }

        _currentReagents = solution.GetReagentPrototypes();
    }
}
