...
public sealed partial class SiliconLawMenu : FancyWindow
{
    ...
    private static readonly TimeSpan PressCooldown = TimeSpan.FromSeconds(2);
    private static readonly TimeSpan ChatCooldown = TimeSpan.FromSeconds(1);
    ...
    private void OnLawAnnounceButtonPressed(BaseButton.ButtonEventArgs obj)
    {
        ...
        for (int i = 0; i < toAnnounce.Count; i++)
        {
            _queuedLaws.Enqueue((toAnnounce[i], _timing.CurTime + (ChatCooldown * i)));
        }

        // Disable button until all laws announced, plus an additional cooldown
        _nextAllowedAnnouncePress = _timing.CurTime + ChatCooldown * toAnnounce.Count + PressCooldown;
    }
