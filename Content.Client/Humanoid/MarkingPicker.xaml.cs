...
public sealed partial class MarkingPicker : Control
{
    ...
    public void Populate(string filter)
    {
        ...
        foreach (var marking in sortedMarkings)
        {
            ...
            Texture previewSprite = marking.BodyPart == HumanoidVisualLayers.Tail
                    ? _sprite.RsiStateLike(marking.Sprites[0]).TextureFor(Direction.North)
                    : _sprite.Frame0(marking.Sprites[0]);
            var item = CMarkingsUnused.AddItem($"{GetMarkingName(marking)}", previewSprite );
    ...
    public void PopulateUsed()
    {
        ...
        foreach (var marking in _currentMarkings.GetReverseEnumerator(_selectedMarkingCategory))
        {
            ...
            Texture previewSprite = newMarking.BodyPart == HumanoidVisualLayers.Tail
                ? _sprite.RsiStateLike(newMarking.Sprites[0]).TextureFor(Direction.North)
