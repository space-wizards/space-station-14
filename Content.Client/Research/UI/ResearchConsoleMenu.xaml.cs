...
namespace Content.Client.Research.UI
{
    ...
    public sealed partial class ResearchConsoleMenu : DefaultWindow
    {
        ...
        private void UnlockableTechnologySelected(ItemList.ItemListSelectedEventArgs obj)
        {
            ...
            UnlockButton.Disabled = Owner.Points < TechnologySelected.Value.RequiredPoints;
        ...
        public void PopulateSelectedTechnology()
        {
            ...
            TechnologyIcon.Texture = TechnologySelected.Value.Icon.Frame0();
            TechnologyName.Text = TechnologySelected.Value.Name;
            TechnologyDescription.Text = TechnologySelected.Value.Description + $"\n{TechnologySelected.Value.RequiredPoints} " + Loc.GetString("research-console-menu-research-points-text" ,("points", Owner.Points)).ToLowerInvariant();
            TechnologyRequirements.Text = Loc.GetString("research-console-tech-requirements-none");

            var prototypeMan = IoCManager.Resolve<IPrototypeManager>();

            for (var i = 0; i < TechnologySelected.Value.RequiredTechnologies.Count; i++)
            {
                var requiredId = TechnologySelected.Value.RequiredTechnologies[i];
                if (!prototypeMan.TryIndex(requiredId, out TechnologyPrototype? prototype)) continue;
                if (i == 0)
                    TechnologyRequirements.Text = Loc.GetString("research-console-tech-requirements-prototype-name", ("prototypeName", prototype.Value.Name));
                else
                    TechnologyRequirements.Text += $", {prototype.Value.Name}";
            }
        }
