    ...
    public sealed partial class HealthAnalyzerWindow : DefaultWindow
    {
        ...
        public void Populate(HealthAnalyzerScannedUserMessage msg)
        {
            ...
            if (msg.TargetEntity != null && entities.TryGetComponent<DamageableComponent>(msg.TargetEntity, out var damageable))
            {
                ...
                {
                    text.Append($"\n{Loc.GetString("health-analyzer-window-damage-group-text", ("damageGroup", Loc.GetString("health-analyzer-window-damage-group-" + damageGroupId.ToLower())), ("amount", damageAmount))}");
                    ...
                    foreach (var type in group.DamageTypes)
                    {
                        if (DamagePerType.TryGetValue(type, out var typeAmount))
                        {
                            ...
                            {
                                shownTypes.Add(type);
                                text.Append($"\n- {Loc.GetString("health-analyzer-window-damage-type-text", ("damageType", Loc.GetString("health-analyzer-window-damage-type-" + type.ToLower())), ("amount", typeAmount))}");
                            }
                        }
                    }
                    text.AppendLine();
