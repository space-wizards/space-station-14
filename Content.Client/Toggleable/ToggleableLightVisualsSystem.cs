...
public sealed class ToggleableLightVisualsSystem : VisualizerSystem<ToggleableLightVisualsComponent>
{
    ...
    {
        if (!AppearanceSystem.TryGetData(uid, ToggleableLightVisuals.Enabled, out bool enabled, args.Component))
            return;

        var modulate = AppearanceSystem.TryGetData(uid, ToggleableLightVisuals.Color, out Color color, args.Component);
    ...
    private void OnGetEquipmentVisuals(EntityUid uid, ToggleableLightVisualsComponent component, GetEquipmentVisualsEvent args)
    {
        if (!TryComp(uid, out AppearanceComponent? appearance)
            || !AppearanceSystem.TryGetData(uid, ToggleableLightVisuals.Enabled, out bool enabled, appearance)
            || !enabled)
            return;
        ...
        var modulate = AppearanceSystem.TryGetData(uid, ToggleableLightVisuals.Color, out Color color, appearance);
