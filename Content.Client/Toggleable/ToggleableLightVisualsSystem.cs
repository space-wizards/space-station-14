...
public sealed class ToggleableLightVisualsSystem : VisualizerSystem<ToggleableLightVisualsComponent>
{
    ...
    {
        if (!AppearanceSystem.TryGetData(uid, ToggleableLightVisuals.Enabled, out bool enabled))
            return;

        var modulate = AppearanceSystem.TryGetData(uid, ToggleableLightVisuals.Color, out Color color);
    ...
    private void OnGetEquipmentVisuals(EntityUid uid, ToggleableLightVisualsComponent component, GetEquipmentVisualsEvent args)
    {
        if (!TryComp(uid, out AppearanceComponent? appearance)
            || !AppearanceSystem.TryGetData(uid, ToggleableLightVisuals.Enabled, out bool enabled)
            || !enabled)
            return;
