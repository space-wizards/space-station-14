...
public sealed partial class AirAlarmWindow : FancyWindow
{
    ...
    private Dictionary<string, HeatPumpControl> _heatPumps = new();
    ...
    public AirAlarmWindow()
    {
        ...
        _tabContainer.SetTabTitle(3, Loc.GetString("air-alarm-ui-window-tab-heat-pumps"));
        ...
        _resyncDevices.OnPressed += _ =>
        {
            ...
            _sensors.Clear();
            CHeatPumpContainer.RemoveAllChildren();
            _heatPumps.Clear();
    ...
    public void UpdateDeviceData(string addr, IAtmosDeviceData device)
    {
        switch (device)
        {
            ...
            case AtmosSensorData sensor:
                ...
                break;
            case GasHeatPumpData heatPump:
                if (!_heatPumps.TryGetValue(addr, out var heatPumpControl))
                {
                    var control = new HeatPumpControl(heatPump, addr);
                    control.HeatPumpDataChanged += AtmosDeviceDataChanged;
                    control.HeatPumpDataCopied += AtmosDeviceDataCopied;
                    _heatPumps.Add(addr, control);
                    CHeatPumpContainer.AddChild(control);
                }
                else
                {
                    heatPumpControl.ChangeData(heatPump);
                }
