...
{
    public event Action<DeviceAddress, IAtmosDeviceData>? AtmosDeviceDataChanged;
	public event Action<IAtmosDeviceData>? AtmosDeviceDataCopied;
    public event Action<LocDeviceAddress, AtmosMonitorThresholdType, AtmosAlarmThreshold, Gas?>? AtmosAlarmThresholdChanged;
    ...
    private Dictionary<DeviceAddress, PumpControl> _pumps = new();
    private Dictionary<DeviceAddress, ScrubberControl> _scrubbers = new();
    private Dictionary<DeviceAddress, SensorInfo> _sensors = new();
    ...
    {
        _address.SetMarkup(state.Address.ToString());
    ...
    public void UpdateDeviceData(LocDeviceAddress addr, IAtmosDeviceData device)
    {
        switch (device)
        {
            case GasVentPumpData pump:
                if (!_pumps.TryGetValue(addr.AddressId, out var pumpControl))
                {
                    var control = new PumpControl(pump, addr);
                    control.PumpDataChanged += AtmosDeviceDataChanged;
                    control.PumpDataCopied += AtmosDeviceDataCopied;
                    _pumps.Add(addr.AddressId, control);
            ...
            case AtmosMonitorData sensor:
                if (!_sensors.TryGetValue(addr.AddressId, out var sensorControl))
                {
                    ...
                    _sensors.Add(addr.AddressId, control);
