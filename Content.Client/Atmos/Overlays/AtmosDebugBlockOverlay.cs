...
{
    public sealed class AtmosDebugBlockOverlay : Overlay
    {
                        ...
                        if (dataMaybeNull != null)
                        {
                            ...
                            if (pass == 0)
                            {
                                ...
                                switch (_atmosDebugOverlaySystem.CfgMode & (
                                            AtmosDebugShowMode.TotalMoles | AtmosDebugShowMode.GasMoles
                                            | AtmosDebugShowMode.Temperature
                                        )) {
                                    case AtmosDebugShowMode.TotalMoles:
                                        foreach (float f in data.Moles)
                                        {
                                            total += f;
                                        }
                                        break;
                                    case AtmosDebugShowMode.GasMoles:
                                        total = data.Moles[_atmosDebugOverlaySystem.CfgSpecificGas];
                                        break;
                                    case AtmosDebugShowMode.Temperature:
