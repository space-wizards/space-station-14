...
namespace Content.Client.Credits
{
    ...
    public sealed partial class CreditsWindow : DefaultWindow
    {
        ...
        private void PopulatePatrons(BoxContainer patronsContainer)
        {
			...
			patronsContainer.AddChild(new Label {StyleClasses = {StyleBase.StyleClassLabelHeading}, Text = "Спонсоры Проекта"});
            ...
            foreach (var tier in patrons.GroupBy(p => p.Tier).OrderBy(p => -p.Key))
            {
                ...
                foreach (var patr in tier.OrderBy(p => p.Name))
				{
					var label = new RichTextLabel();
					var msg = $"[color={patr.Color}]{patr.Name}[/color] - [italic][color={patr.TierColor}]{patr.TierName}[/color][/italic]";
					
					if (_sponsorManager.IsSponsor(patr.InGame))
						msg = $"[bold]{msg}[/bold]";
					
					label.SetMessage(FormattedMessage.FromMarkup(msg));
					
					if (!first)
						patronsContainer.AddChild(new Control {MinSize = new Vector2(0, 4)});
					first = false;
        ...
        private IEnumerable<PatronEntry> LoadPatrons()
        {
            ...
            return sequence
                ...
                .Select(m => new PatronEntry(m["Name"].AsString(), m["InGame"].AsString(), m["Tier"].AsInt(), m["TierName"].AsString(), m["Color"].AsString(), m["TierColor"].AsString()));
        }
        ...
        private sealed class PatronEntry
        {
			...
			public string InGame { get; }
            public int Tier { get; }
            public string TierName { get; }
            public string Color { get; }
            public string TierColor { get; }

            public PatronEntry(string name, string inGame, int tier, string tierName, string color, string tierColor)
            {
                Name = name;
				InGame = inGame;
                Tier = tier;
