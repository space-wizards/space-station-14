...
public sealed partial class CreditsWindow : DefaultWindow
{
    ...
    private const int AttributionsSourcesPerPage = 30;
    ...
    private void PopulateAttributions(BoxContainer attributionsContainer, int count)
    {
    ...
    private void CollectRSiAttributions()
    {
        ...
        foreach (var stream in rsiStreams)
        {
            ...
            try
            {
                ...
                var files = states.Select(n => (MappingDataNode)n)
                    .Select(n => stream.Directory.ToString().Split("/").Last() + "/" + n.Get("name") + ".png");

                m.AddMarkupPermissive(Loc.GetString("credits-window-attributions-files",
                    ("files", string.Join(", ", files))));
                m.AddText("\n");
                m.AddMarkupPermissive(Loc.GetString("credits-window-attributions-copyright",
                    ("copyright", copyright)));
                m.AddText("\n");
            }
            catch (Exception e)
            {
                m.AddMarkupPermissive(Loc.GetString("credits-window-attributions-failed", ("file", stream.ToString())));
                m.AddText("\n");
                Logger.GetSawmill("Credits").Error($"{stream.ToString()}\n{e}");
            }
    ...
    private void CollectRgaAttributions()
    {
            ...
            try
            {
                ...
                foreach (var attribution in sequence.Sequence)
                {
                    ...
                    m.AddMarkupPermissive(Loc.GetString("credits-window-attributions-copyright",
                        ("copyright", copyright)));
                    m.AddText("\n");
                    m.AddMarkupPermissive(Loc.GetString("credits-window-attributions-license", ("license", license)));
                    m.AddText("\n");
                    m.AddMarkupPermissive(Loc.GetString("credits-window-attributions-source", ("source", source)));
                    m.AddText("\n");
