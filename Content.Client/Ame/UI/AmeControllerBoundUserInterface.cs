...
public sealed partial class AmeControllerBoundUserInterface : BoundUserInterface
{
    ...
    public override void Update()
    {
        ...
        if (_nodeContainer.TryGetFirstNodeGroup<AmeNodeGroupComponent>(Owner, out var group) && group.Value.Comp.Cores.Count > 0)
        {
            isMaster = group.Value.Comp.MasterController == Owner;
            coreCount = group.Value.Comp.Cores.Count;
            targetedPowerSupply = _ameHandler.CalculatePower(controller.InjectionAmount, group.Value.Comp.Cores.Count) / 1000;
        }
