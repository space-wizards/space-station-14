...
namespace Content.Client.Ame.UI
{
    ...
    public sealed partial class AmeWindow : DefaultWindow
    {
        ...
        public void UpdateState(BoundUserInterfaceState state)
        {
            ...
            SetAlertStatus(AlertStatus, castState.CoreCount, castState.InjectionAmount);
        }

        private void SetAlertStatus(Label alertStatus, int coreCount, int injectionCount)
        {
            var safeInjection = coreCount * 2;

            if (coreCount == 0 || injectionCount == safeInjection)
            {
                alertStatus.Visible = false;
                return;
            }

            if (injectionCount < safeInjection)
                AlertStatus.Text = Loc.GetString("ame-window-engine-injection-status-underloaded-label");
            else if (injectionCount > safeInjection)
                AlertStatus.Text = Loc.GetString("ame-window-engine-injection-status-overloaded-label");
