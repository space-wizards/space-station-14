...
public sealed partial class PowerMonitoringWindow : FancyWindow
{
        ...
        EntityCoordinates? monitorCoords)
    {
        ...
        {
            foreach (var data in _navMapTracking.TrackingData)
            {
                AddTrackedEntityToNavMap(data, _trackedEntity != null);
    ...
    private void AddTrackedEntityToNavMap(NavMapTrackingData data, bool useDarkColors = false)
    {
        if (!NavMap.Visible)
            return;

        data.Modulate = _entManager.GetEntity(data.NetEntity) != _trackedEntity && useDarkColors ? Color.DimGray : Color.White;
        data.Blinks = _entManager.GetEntity(data.NetEntity) == _trackedEntity;

        // We expect a single tracked entity at a given coordinate
        NavMap.TrackedEntities[_entManager.GetCoordinates(data.Coordinates)] = data;
    }
