...
public sealed partial class GeneratorWindow : FancyWindow
{
    ...
    public void Update(PortableGeneratorComponentBuiState state)
    {
        ...
        var canSwitch = _entityManager.TryGetComponent(_entity, out VoltageTogglerComponent? voltageToggler);
        var toggler = _entityManager.System<SharedVoltageTogglerSystem>();
        OutputSwitchLabel.Visible = canSwitch;
        OutputSwitchButton.Visible = canSwitch;

        if (voltageToggler != null)
        {
            var voltage = toggler.VoltageString(toggler.GetVoltage((_entity, voltageToggler)));
            OutputSwitchLabel.Text = Loc.GetString("portable-generator-ui-current-output", ("voltage", voltage));
            var nextVoltage = toggler.VoltageString(toggler.GetNextVoltage((_entity, voltageToggler)));
