...
public sealed partial class GuideMicrowaveEmbed : PanelContainer, IDocumentTag, ISearchableControl, IPrototypeRepresentationControl
{
    ...
    {
        var spawnEffect = reaction.Effects.OfType<SpawnEntity>().First();
        var result = _prototype.Index<EntityPrototype>(spawnEffect.Entity);

        RepresentedPrototype = result;
        IconContainer.AddChild(new GuideEntityEmbed(spawnEffect.Entity.Id, false, false));
        ResultName.SetMarkup(result.Name);
        ResultDescription.SetMarkup(result.Description);

        MicrowaveDetails.Visible = false;
        ReactionContainer.Visible = true;
        ReactionContainer.AddChild(new GuideReagentReaction(reaction, _prototype, _systemManager));
    }
    // DS14-end
