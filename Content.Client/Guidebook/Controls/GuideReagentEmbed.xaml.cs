...
public sealed partial class GuideReagentEmbed : BoxContainer, IDocumentTag, ISearchableControl
{
    ...
    {
        if (!_chemistryRegistry.TryIndexPrototype(reagent, out var reagentProto))
            return;
        GenerateControl(reagentProto.ReagentDefinition);
    }

    public GuideReagentEmbed(ReagentDefinitionComponent reagent) : this()
    {
    ...
    public bool TryParseTag(Dictionary<string, string> args, [NotNullWhen(true)] out Control? control)
    {
        ...
        if (!_chemistryRegistry.TryIndexPrototype(id, out var reagent))
        {
            Logger.Error($"Specified reagent \"{id}\" is not a valid reagent");
            return false;
        }

        GenerateControl(reagent.ReagentDefinition);

        control = this;
        return true;
    }

    private void GenerateControl(ReagentDefinitionComponent reagent)
    {
    ...
    private void GenerateSources(ReagentDefinitionComponent reagent)
    {
