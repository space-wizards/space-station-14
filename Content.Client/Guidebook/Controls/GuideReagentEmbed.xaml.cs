    ...
    private void GenerateControl(ReagentPrototype reagent)
    {
        ...
        if (reagent.AllowedJobs.Count > 0 || reagent.AllowedDepartments.Count > 0)
        {
            ...
            description.AddMarkupPermissive(
                _contraband.GenerateDepartmentExamineMessage(reagent.AllowedDepartments, reagent.AllowedJobs));
        }
        // Other contraband text
        else if (reagent.ContrabandSeverity != null && _prototype.TryIndex(reagent.ContrabandSeverity.Value, out var severity))
        {
            description.PushNewline();
