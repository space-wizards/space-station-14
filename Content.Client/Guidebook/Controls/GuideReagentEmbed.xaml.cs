...
public sealed partial class GuideReagentEmbed : BoxContainer, IDocumentTag, ISearchableControl
{
    ...
    private void GenerateSources(ReagentPrototype reagent)
    {
        ...
        var reactionSources = _prototype.EnumeratePrototypes<ReactionPrototype>()
            .Where(p => p.Source && p.Products.ContainsKey(reagent.ID))
            .OrderBy(p => p.Priority)
            .ThenBy(p => p.Products.Count)
            .ToList();

        foreach (var reactionPrototype in reactionSources)
        {
            SourcesDescriptionContainer.AddChild(new GuideReagentReaction(reactionPrototype, _prototype, _systemManager));
            sourceCount++;
        }

        // store the names of the entities used so we don't get repeats.
        var usedNames = new List<string>();
        foreach (var entProto in _prototype.EnumeratePrototypes<EntityPrototype>())
        {
            ...
            if (extractableComponent.JuiceSolution?.ContainsPrototype(reagent.ID) == true)
            {
                SourcesDescriptionContainer.AddChild(new GuideReagentReaction(
                    entProto,
                    extractableComponent.JuiceSolution,
                    "DefaultJuice",
                    _prototype,
                    _systemManager));
            ...
            if (extractableComponent.GrindableSolution is { } grindableSolutionId &&
                ...
                grindableSolution.ContainsPrototype(reagent.ID))
            {
                SourcesDescriptionContainer.AddChild(new GuideReagentReaction(
                    entProto,
                    grindableSolution,
        ...
        SourcesContainer.Visible = sourceCount != 0;
    }
}
