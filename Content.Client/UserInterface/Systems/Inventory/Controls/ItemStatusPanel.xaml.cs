...
public sealed partial class ItemStatusPanel : Control
{
    ...
    private void BuildNewEntityStatus()
    {
        ...
        ItemStatusEntry? overridingStatus = null;
        var overridingPriority = int.MinValue;
        foreach (var status in collectMsg.Controls)
        {
            if (!_prototypeManager.Resolve(status.Prototype, out var prototype))
                return;

            if (!prototype.Override || overridingStatus != null && prototype.Priority <= overridingPriority)
                continue;

            overridingStatus = status;
            overridingPriority = prototype.Priority;
        }

        if (overridingStatus is { } selectedStatus)
        {
            StatusContents.AddChild(selectedStatus.Control);
            return;
        }

        foreach (var status in collectMsg.Controls.OrderByDescending(status => _prototypeManager.Index(status.Prototype).Priority))
        {
