...
public sealed partial class ItemStatusPanel : Control
{
    ...
    private void BuildNewEntityStatus()
    {
        ...
        var filtered = FilterConflicting(collectMsg.Controls);
        foreach (var status in filtered)
        {
            StatusContents.AddChild(status.Control);
        }
    }

    /// <summary>
    /// Removes statuses that conflict with higher-priority statuses via whitelists or blacklists.
    /// </summary>
    private List<ItemStatusEntry> FilterConflicting(List<ItemStatusEntry> controls)
    {
        var sorted = controls
            .OrderByDescending(status => _prototypeManager.Index(status.Prototype).Priority)
            .ToList();

        var removed = new HashSet<ProtoId<ItemStatusPrototype>>();
        foreach (var status in sorted)
        {
            if (removed.Contains(status.Prototype))
                continue;

            if (!_prototypeManager.Resolve(status.Prototype, out var prototype))
                continue;

            if (prototype.Blacklist != null)
            {
                foreach (var other in sorted)
                {
                    if (other.Prototype != status.Prototype && prototype.Blacklist.Contains(other.Prototype))
                        removed.Add(other.Prototype);
                }
            }
            ...
            if (prototype.Whitelist != null)
            {
                foreach (var other in sorted)
                {
                    if (other.Prototype != status.Prototype && !prototype.Whitelist.Contains(other.Prototype))
                        removed.Add(other.Prototype);
                }
            }
        }

        return [.. sorted.Where(status => !removed.Contains(status.Prototype))];
    }
}
