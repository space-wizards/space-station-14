        ...
        public void PopulateMaterials(EntityUid lathe)
        {
            ...
            foreach (var (materialId, volume) in materials.Storage)
            {
                ...
                if (material.StackEntity != null)
                {
                    ...
                    var multipler = composition.MaterialComposition.FirstOrDefault(kvp => kvp.Key == materialId).Value;
                    maxEjectableSheets = (int) MathF.Floor(volume / multipler);
                }

                var name = Loc.GetString(material.Name);

                var row = new LatheMaterialEjector(materialId, OnEjectPressed, maxEjectableSheets)
                {
