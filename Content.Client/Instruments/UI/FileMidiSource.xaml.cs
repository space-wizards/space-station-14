...
public sealed partial class FileMidiSource : InstrumentMidiSourceBase
{
    ...
    private async void OnAddButtonPressed(ButtonEventArgs obj)
    {
        try
        {
            ...
            var trackPath = new ResPath(DateTime.Now.Ticks + ".midi");
            await _midiCollection.AddMidiFile(trackPath, file);
        }
        catch
        {
    ...
    private void OnRenameButtonPressed(ButtonEventArgs obj)
    {
        ...
        if (trackPath.Value.FilenameWithoutExtension.Length == 0)
            return;
        ...
        var entry = new QuickDialogEntry(field, QuickDialogEntryType.ShortText, prompt, trackPath.Value.FilenameWithoutExtension);
        ...
        _reasonDialog.OnConfirmed += responses =>
        {
            ...
            if (!newName.EndsWith(".midi") && !newName.EndsWith(".mid"))
                newName += ".midi";
            if (!_midiCollection.RenameMidiFile(trackPath.Value, new ResPath(newName)))
                _userInterfaceManager.Popup(Loc.GetString("instruments-component-menu-files-error"));
        };
