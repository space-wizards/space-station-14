...
namespace Content.Client.Administration.UI.ManageSolutions
{
    ...
    public sealed partial class EditSolutionsWindow : DefaultWindow
    {
        ...
        private readonly SharedSolutionSystem _solutionSystem;

        private NetEntity _target = NetEntity.Invalid;
        private string? _selectedSolution;
        private AddReagentWindow? _addReagentWindow;
        private Dictionary<string, Entity<SolutionComponent>>? _solutions;
        ...
        public void UpdateReagents()
        {
            ...
            if (!_solutions.TryGetValue(_selectedSolution, out var solutionId) ||
                ...
                return;
            var solution = (solutionId, solutionComp);
            UpdateVolumeBox(solution);
            UpdateThermalBox(solution);

            foreach (var reagent in _solutionSystem.EnumerateReagents(solution, true))
            {
        ...
        private void UpdateVolumeBox(Entity<SolutionComponent> solution)
        {
            ...
            volumeLabel.Text = Loc.GetString("admin-solutions-window-volume-label",
                ("currentVolume", solution.Comp.Volume),
            ...
            capacitySpin.Value = (float) solution.Comp.MaxVolume;
        ...
        /// </summary>
        /// <param name="solution">The selected solution.</param>
        private void UpdateThermalBox(Entity<SolutionComponent> solution)
        {
            ThermalBox.DisposeAllChildren();
            var heatCap = solution.Comp.HeatCapacity;
            ...
            specificHeatLabel.Text = Loc.GetString("admin-solutions-window-specific-heat-label",
                ("specificHeat",
                (heatCap/solution.Comp.Volume.Float()).ToString("G3")));
            ...
            heatCapacityLabel.Text = Loc.GetString("admin-solutions-window-heat-capacity-label",
                ("heatCapacity", heatCap.ToString("G3")));
            ...
            temperatureSpin.Value = solution.Comp.Temperature;
            ...
            thermalEnergySpin.Value = solution.Comp.ThermalEnergy;
        ...
        private void AddReagentEntry(ReagentQuantity reagentQuantity)
        {
            ...
            spin.OnValueChanged += (args) => SetReagent(args, reagentQuantity.ReagentDef);
            spin.HorizontalExpand = true;

            box.AddChild(new Label() { Text = reagentQuantity.ReagentDef.Id , HorizontalExpand = true});
            box.AddChild(spin);
        ...
        /// </summary>
        private void SetReagent(FloatSpinBox.FloatSpinBoxEventArgs args, ReagentDef reagent)
        {
            if (!reagent.IsValid || _solutions == null || _selectedSolution == null ||
                !_solutions.TryGetValue(_selectedSolution, out var solution))
                return;

            var current = _solutionSystem.GetTotalQuantity(solution, reagent);
            var delta = args.Value - current.Float();

            if (MathF.Abs(delta) < 0.01)
                return;

            var command = $"addreagent {_target} {_selectedSolution} {reagent.Id} {delta}";
        ...
        public void UpdateSolutions(List<(string, NetEntity)>? solutions)
        {
            ...
            var solutionQuery = _entityManager.GetEntityQuery<SolutionComponent>();
            if (solutions is { Count: > 0 })
            {
                ...
                {
                    if (_entityManager.TryGetEntity(netSolution, out var solEnt)
                        && solutionQuery.TryComp(solEnt, out var solComp))
                        _solutions.Add(name, (solEnt.Value, solComp));
                }
            }
            else
                _solutions = null;
