...
namespace Content.Client.Administration.UI.ManageSolutions
{
    ...
    public sealed partial class EditSolutionsWindow : DefaultWindow
    {
        ...
        /// </summary>
        private void AddReagentEntry(string id, FixedPoint2 quantity)
        {
            var box = new BoxContainer();
            var spin = new FloatSpinBox(1, 2);

            spin.Value = quantity.Float();
            spin.OnValueChanged += (args) => SetReagent(args, id);
            spin.HorizontalExpand = true;

            box.AddChild(new Label() { Text = id, HorizontalExpand = true});
            box.AddChild(spin);
