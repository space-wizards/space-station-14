...
namespace Content.Client.Administration.UI.ManageSolutions
{
    ...
    public sealed partial class AddReagentWindow : DefaultWindow
    {
        ...
        {
            if (_selectedReagentId == null)
                return;

            var quantity = _quantitySpin.Value.ToString("F2");
            var command = $"addreagent {_targetEntity} {_targetSolution} {_selectedReagentId} {quantity}";
            _consoleHost.ExecuteCommand(command);
        }

        private void ReagentListSelected(ItemList.ItemListSelectedEventArgs obj)
        {
            _selectedReagentId = (string) obj.ItemList[obj.ItemIndex].Metadata!;
            UpdateAddButton();
        }
        ...
        {
            ReagentList.Clear();
            foreach (var reagent in _entity.System<ChemistryRegistrySystem>().EnumerateReagents())
            {
                if (!string.IsNullOrEmpty(filter) &&
                   !reagent.Comp.Id.ToLowerInvariant().Contains(filter.Trim().ToLowerInvariant()))
                {
                    continue;
                }
                ...
                {
                    Metadata = reagent.Comp.Id,
                    Text = reagent.Comp.Id,
                };
