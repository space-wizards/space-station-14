...
public partial class AdminLogsWindow : SS14Window
{
    ...
    {
        foreach (var control in LogsContainer.Children)
        {
            if (control is not AdminLogLabel log)
            {
                continue;
            }

            log.Visible = ShouldShowLog(log);
        }
    }
    ...
    private bool ShouldShowLog(AdminLogLabel label)
    {
        return label.Text != null &&
               label.Text.Contains(LogSearch.Text, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void AddLogs(SharedAdminLog[] logs)
    {
        for (var i = 0; i < logs.Length; i++)
        {
            ...
            var label = new AdminLogLabel(log.Id)
            {
                Text = log.Message
            };
