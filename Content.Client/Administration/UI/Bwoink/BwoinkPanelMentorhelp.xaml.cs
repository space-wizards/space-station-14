...
public sealed partial class BwoinkPanelMentorhelp : BoxContainer
{
    ...
    public BwoinkPanelMentorhelp(Action<string> messageSender)
    {
        RobustXamlLoader.Load(this);

        var msg = new FormattedMessage();
        msg.PushColor(Color.DarkCyan);
    ...
    public void ReceiveLine(SharedBwoinkAdminSystem.BwoinkTextMessage message)
    {
        if (!Visible)
            Unread++;

        var formatted = new FormattedMessage(1);
        formatted.AddMarkupOrThrow($"[color=darkcyan]{message.SentAt.ToShortTimeString()}[/color] {message.Text}");
