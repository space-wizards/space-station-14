...
namespace Content.Client.Nuke
{
    ...
    public sealed partial class NukeMenu : DefaultWindow
    {
        ...
        public void UpdateState(NukeUiState state)
        {
            ...
            var unknown = Loc.GetString("generic-unknown-title");

            var remainingTime = unknown;
            if (state.RemainingTime != null)
                remainingTime = state.RemainingTime.Value.ToString(@"hh\:mm\:ss");

            var cooldownTime = unknown;
            if (state.CooldownTime != null)
                cooldownTime = state.CooldownTime.Value.ToString(@"hh\:mm\:ss");
            ...
            switch (state.Status)
            {
                ...
                case NukeStatus.AWAIT_ARM:
                    ...
                    secondMsg = Loc.GetString("nuke-user-interface-second-status-time",
                        ("time", state.ArmingTime.ToString(@"hh\:mm\:ss")));
                    break;
