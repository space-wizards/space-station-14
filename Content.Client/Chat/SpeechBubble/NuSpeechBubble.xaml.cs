...
public sealed partial class NuSpeechBubble : Control
{
    ...
    public NuSpeechBubble(ChatMessage message, UI.SpeechBubble.SpeechType type, EntityUid senderEntity, Color? fontColor = null)
    {
        ...
        //I can't remember the syntax to add more than on in xaml
        //TODO: xaml
        Label.StyleClasses = [ "bubbleContent", "fontChat" ];
        LabelPanel.StyleClasses = ["speechBox", styleClass ];

        //TODO: Kill bubbleContent in chat code so we don't have to regex everything ever made
        switch (type)
        {
            case  UI.SpeechBubble.SpeechType.Emote:
                Label.SetMessage(FormatSpeech(message.WrappedMessage, Color.White));
                break;
            case UI.SpeechBubble.SpeechType.Looc:
                Label.SetMessage(FormatSpeech(message.WrappedMessage, Color.FromHex("#48d1cc")));
                break;
            case  UI.SpeechBubble.SpeechType.Whisper:
                Label.SetMessage(ExtractAndFormatSpeechSubstring(message, "BubbleContent", fontColor));
                break;
            case  UI.SpeechBubble.SpeechType.Say:
                Label.SetMessage(ExtractAndFormatSpeechSubstring(message, "BubbleContent", fontColor));
                break;
    ...
    private static string GetStyle(UI.SpeechBubble.SpeechType type)
    {
        return type switch
        {
            UI.SpeechBubble.SpeechType.Emote => "emoteBox",
            UI.SpeechBubble.SpeechType.Whisper => "whisperBox",
            UI.SpeechBubble.SpeechType.Looc => "emoteBox",
            _ => "sayBox",
        };
    }
