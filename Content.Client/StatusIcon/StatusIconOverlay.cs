...
public sealed class StatusIconOverlay : Overlay
{
    ...
    {
        DrawMobStatusIcons(args);
        DrawNonMobStatusIcons(args);
    }

    public void DrawMobStatusIcons(in OverlayDrawArgs args)
    {
    ...
    public void DrawNonMobStatusIcons(in OverlayDrawArgs args)
    {
        var handle = args.WorldHandle;
        ...
        var query = _entity
            .AllEntityQueryEnumerator<StatusIconNonMobComponent, SpriteComponent, TransformComponent, MetaDataComponent>();
        while (query.MoveNext(out var uid, out var comp, out var sprite, out var xform, out var meta))
        {
            if (xform.MapID != args.MapId || !sprite.Visible)
                continue;
