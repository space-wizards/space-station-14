...
public sealed class COTelekinesisHandSystem : COSharedTelekinesisHandSystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<COTelekinesisTetherComponent, ComponentStartup>(OnTetheredStartup);
        SubscribeLocalEvent<COTelekinesisTetherComponent, ComponentShutdown>(OnTetheredShutdown);
        SubscribeLocalEvent<COTelekinesisHandComponent, AfterAutoHandleStateEvent>(OnAfterState);
        _overlay.AddOverlay(new COTelekinesisTetherOverlay(EntityManager));
    }

    private void OnAfterState(EntityUid uid, COTelekinesisHandComponent component, ref AfterAutoHandleStateEvent args)
    {
    ...
    public override void Update(float frameTime)
    {
        ...
        RaisePredictiveEvent(new CORequestTetherMoveEvent()
        {
            Coordinates = GetNetCoordinates(coords)
        });
    }

    private void OnTetheredStartup(EntityUid uid, COTelekinesisTetherComponent component, ComponentStartup args)
    {
        ...
        if (TryComp<COTelekinesisHandComponent>(component.Tetherer, out var tether))
        {
            sprite.Color = tether.LineColor;
        }
    }

    private void OnTetheredShutdown(EntityUid uid, COTelekinesisTetherComponent component, ComponentShutdown args)
    {
        if (!TryComp<SpriteComponent>(uid, out var sprite))
            return;
