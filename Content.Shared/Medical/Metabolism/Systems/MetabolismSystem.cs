...
public sealed class MetabolismSystem : EntitySystem
{
    ...
    private void OnBodyInit(EntityUid uid, MetabolismComponent respirator, BodyInitializedEvent args)
    {
        if (!respirator.UsesBodySolution)
            return;
        UpdateCachedSolutions((uid, respirator), args.Body);
    }
    ...
    private bool TryAbsorbReagents(Entity<MetabolismComponent> metabolism)
    {
        ...
        foreach (var (reagentId, quantity) in metabolism.Comp.CachedAbsorbedReagents)
        {
            ...
            var reactions = solQuant.Float() / reqQuant.Float();
            unitReactions = reactions < unitReactions ? reactions : unitReactions;
        }
        ...
        foreach (var (reagentId, quantity) in metabolism.Comp.CachedAbsorbedReagents)
        {
            var reqQuant = quantity*unitReactions;
