...
public sealed class StickySystem : EntitySystem
{
    ...
    {
        // target is the user when unsticking and the surface when sticking, it will never be null
    ...
    private void StartUnsticking(Entity<StickyComponent> ent, EntityUid user)
    {
        ...
        _doAfter.TryStartDoAfter(new DoAfterArgs(EntityManager, user, comp.UnstickDelay, new StickyDoAfterEvent(), uid, target: user)
        {
            BreakOnMove = true,
            NeedHand = true,
        });
