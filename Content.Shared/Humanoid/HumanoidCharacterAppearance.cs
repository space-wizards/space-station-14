...
public sealed partial class HumanoidCharacterAppearance : ICharacterAppearance, IEquatable<HumanoidCharacterAppearance>
{
    ...
    public static HumanoidCharacterAppearance Random(string species, Sex sex)
    {
        ...
        // declare some defaults. ensures that the hair and eyes on hues-colored species don't match the skin. 
        ...
        switch (skinType)
        {
            ...
            case HumanoidSkinColor.HumanToned:
                ...
                // pick a random realistic hair color from the list, randomize it juuuuust a little bit,
        ...
        foreach (var category in Enum.GetValues<MarkingCategories>())
        {
            ...
            {
                // roll a die. currently a 1 in 3 chance per category, except Tails, which are 1 in 2 (because of how important they are for spiders and moths.)
                int diceRoll;
                if (category == MarkingCategories.Tail)
                    diceRoll = random.Next(2); // 1d2, 50%
                else
                    diceRoll = random.Next(3); // 1d3, 33%
                ...
                if (diceRoll == 0)
                {
                    ...
                    for (var i = 0; i < loops; i++)
                    {
                        ...
                        // otherwise, add it to the final list and ensure it doesn't get duplicated.
                        newMarkings.Add(markingToAdd);
                        lastMarking = protoToAdd;
                    }
                }
        ...
        List<Color> GetComplementaryColors(Color color, double angle)
        {
            ...
            var second = Color.FromHsl(positiveHSL);
            var third = Color.FromHsl(negativeHSL);

            var palette = new List<Color> { color, second, third };
            return palette;
        }
