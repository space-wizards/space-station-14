...
public abstract class SharedHumanoidAppearanceSystem : EntitySystem
{
    ...
    private void OnInit(EntityUid uid, HumanoidAppearanceComponent humanoid, ComponentInit args)
    {
        ...
        if (string.IsNullOrEmpty(humanoid.Initial)
            || !Proto.TryIndex(humanoid.Initial, out HumanoidProfilePrototype? startingSet))
        {
    ...
    {
        if (!Resolve(uid, ref humanoid) || !Proto.TryIndex<SpeciesPrototype>(species, out var prototype))
        {
            return;
        }
        ...
        humanoid.MarkingSet = new(oldMarkings, prototype.MarkingPoints, _markingManager, Proto);
    ...
    public virtual void SetSkinColor(EntityUid uid, Color skinColor, bool sync = true, bool verify = true, HumanoidAppearanceComponent? humanoid = null)
    {
        ...
        if (!Proto.TryIndex<SpeciesPrototype>(humanoid.Species, out var species))
        {
            return;
        }
    ...
    public virtual void LoadProfile(EntityUid uid, HumanoidCharacterProfile profile, HumanoidAppearanceComponent? humanoid = null)
    {
        ...
        var hairColor = _markingManager.MustMatchSkin(profile.Species, HumanoidVisualLayers.Hair, out var hairAlpha, Proto)
            ? profile.Appearance.SkinColor.WithAlpha(hairAlpha) : profile.Appearance.HairColor;
        var facialHairColor = _markingManager.MustMatchSkin(profile.Species, HumanoidVisualLayers.FacialHair, out var facialHairAlpha, Proto)
            ? profile.Appearance.SkinColor.WithAlpha(facialHairAlpha) : profile.Appearance.FacialHairColor;

        if (_markingManager.Markings.TryGetValue(profile.Appearance.HairStyleId, out var hairPrototype) &&
            _markingManager.CanBeApplied(profile.Species, profile.Sex, hairPrototype, Proto))
        {
            AddMarking(uid, profile.Appearance.HairStyleId, hairColor, false);
        }

        if (_markingManager.Markings.TryGetValue(profile.Appearance.FacialHairStyleId, out var facialHairPrototype) &&
            _markingManager.CanBeApplied(profile.Species, profile.Sex, facialHairPrototype, Proto))
        {
            AddMarking(uid, profile.Appearance.FacialHairStyleId, facialHairColor, false);
        }

        humanoid.MarkingSet.EnsureSpecies(profile.Species, profile.Appearance.SkinColor, _markingManager, Proto);
    ...
    {
        if (Proto.TryIndex<SpeciesPrototype>(speciesId, out var species))
        {
    ...
    {
        if (!Proto.TryIndex<SpeciesPrototype>(species, out var speciesPrototype))
        {
