...
public abstract class SharedHumanoidAppearanceSystem : EntitySystem
{
        ...
        HumanoidAppearanceComponent? targetHumanoid = null)
    {
        ...
        targetHumanoid.EyeGlowing = sourceHumanoid.EyeGlowing;
    ...
    public virtual void LoadProfile(EntityUid uid, HumanoidCharacterProfile? profile, HumanoidAppearanceComponent? humanoid = null)
    {
        ...
        foreach (var marking in profile.Appearance.Markings)
        {
            if (_markingManager.TryGetMarking(marking, out var prototype))
            {
                ...
                {
                    AddMarking(uid, marking.MarkingId, marking.MarkingColors, marking.IsGlowing, false);
                }
                else
                {
        ...
        {
            AddMarking(uid, profile.Appearance.HairStyleId, profile.Appearance.HairGlowing, hairColor, false);
        }

        if (_markingManager.Markings.TryGetValue(profile.Appearance.FacialHairStyleId, out var facialHairPrototype) &&
            _markingManager.CanBeApplied(profile.Species, profile.Sex, facialHairPrototype, _proto))
        {
            AddMarking(uid, profile.Appearance.FacialHairStyleId, profile.Appearance.FacialHairGlowing, facialHairColor, false);
        }
        ...
        foreach (var (marking, prototype) in markingFColored)
        {
            ...
            AddMarking(uid, marking.MarkingId, markingColors, marking.IsGlowing, false);
        }
    ...
    /// <param name="humanoid">Humanoid component of the entity</param>
    public void AddMarking(EntityUid uid, string marking, bool isGlowing, Color? color = null, bool sync = true, bool forced = false, HumanoidAppearanceComponent? humanoid = null)
    {
        if (!Resolve(uid, ref humanoid)
    ...
    public void AddMarking(EntityUid uid, string marking, IReadOnlyList<Color> colors, bool isGlowing, bool sync = true, bool forced = false, HumanoidAppearanceComponent? humanoid = null)
    {
        ...
        var markingObject = new Marking(marking, colors, isGlowing)
        {
            Forced = forced
        };
        humanoid.MarkingSet.AddBack(prototype.MarkingCategory, markingObject);
