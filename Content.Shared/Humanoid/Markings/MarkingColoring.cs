...
public static class MarkingColoring
{
    ...
    )
    {
        ...
        var defaultColor = prototype.Coloring.Default.GetColor(skinColor, eyeColor, speakerColor, markingSet);
        ...
        else
        {
            ...
            for (var i = 0; i < prototype.Sprites.Count; i++)
            {
                ...
                {
                    var marking_color = layerColoring.GetColor(skinColor, eyeColor, speakerColor, markingSet);
...
public sealed partial class LayerColoringDefinition
{
    ...
    public Color GetColor(Color? skin, Color? eyes, Color? speaker, MarkingSet markingSet)
    {
        var color = Type.GetColor(skin, eyes, speaker, markingSet);
        if (color == null)
        {
            foreach (var type in FallbackTypes)
            {
                color = type.GetColor(skin, eyes, speaker, markingSet);
                if (color != null) break;
            }
        }
...
public abstract partial class LayerColoringType
{
    ...
    public abstract Color? GetCleanColor(Color? skin, Color? eyes, Color? speaker, MarkingSet markingSet);
    public Color? GetColor(Color? skin, Color? eyes, Color? speaker, MarkingSet markingSet)
    {
        var color = GetCleanColor(skin, eyes, speaker, markingSet);
        // Negative color
