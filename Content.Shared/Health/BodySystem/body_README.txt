	...
	- BodyPart: This class stores the data for a single body part, such as an arm or a leg. It is also a prototype, so you can define 
	  your own pre-defined body parts (e.g. human_basic arm) in a YAML file. Note that this class has an ID that can be named anything, but 
	...
	as even fully biological creatures have hearts in their chest. Every BodyPart has a size (e.g. humans could have a size 8 torso) that determines how many mechanisms can be fit into
    ...
    Some examples that may or may not exist:
	...
	- Size 7 Internal Fusion Reactor: Too big to fit normally into a human, but you can always remove your kidneys to make room. Creates massive amounts of energy. Mechanical limbs only.
