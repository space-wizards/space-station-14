...
public sealed class EntityFlagSystem : EntitySystem
{
    ...
    public bool HasFlag(EntityUid entityUid, string flagName)
    {
        if (!TryComp<EntityFlagComponent>(entityUid, out var flagComp))
            return false;
        return flagComp.Flags.Contains(flagName) || flagComp.LocalFlags.Contains(flagName);
    }
