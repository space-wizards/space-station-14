...
namespace Content.Shared.Containers.ItemSlots
{
    ...
    /// </summary>
    [ByRefEvent]
    public record struct ItemSlotInsertAttemptEvent(EntityUid SlotEntity, EntityUid Item, ItemSlot Slot, bool Cancelled = false);

    /// <summary>
    /// Event raised on the slot entity and the item being inserted to determine if an item can be ejected from an item slot.
    /// </summary>
    [ByRefEvent]
    public record struct ItemSlotEjectAttemptEvent(EntityUid SlotEntity, EntityUid Item, ItemSlot Slot, bool Cancelled = false);
}
