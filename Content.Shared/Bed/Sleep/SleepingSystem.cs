...
public sealed partial class SleepingSystem : EntitySystem
{
    ...
    [Dependency] private readonly StatusEffectNew.SharedStatusEffectsSystem _sharedStatusEffectNew = default!;
    ...
    public bool TryWaking(Entity<SleepingComponent?> ent, bool force = false, EntityUid? user = null)
    {
        ...
        if (!force && _sharedStatusEffectNew.HasEffectComp<ForcedSleepingStatusEffectComponent>(ent))
        {
            if (user != null)
            {
