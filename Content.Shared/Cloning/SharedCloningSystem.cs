...
public abstract partial class SharedCloningSystem : EntitySystem
{
    ...
    public bool CloneComponents(EntityUid source, EntityUid clone,  HashSet<String> componentsToCopy)
    {
        foreach (var componentName in componentsToCopy)
        {
            ...
            if (!EntityManager.TryGetComponent(source, componentRegistration.Type, out var sourceComp)) // Does the original have this component?
                continue;
            ...
            CopyComp(source, clone, sourceComp);
        }
        return true;
    }
