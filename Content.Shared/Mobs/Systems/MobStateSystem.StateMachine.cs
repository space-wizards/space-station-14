...
public partial class MobStateSystem
{
    ...
    private void ChangeState(EntityUid target, MobStateComponent component, MobState newState, EntityUid? origin = null)
    {
        ...
        if (origin != null && HasComp<ActorComponent>(origin) && HasComp<ActorComponent>(target) && oldState < newState)
            _adminLogger.AddStructured(LogType.Damaged, LogImpact.High, $"{origin:player} caused {target:player} state to change from {oldState} to {newState}");
        else
            _adminLogger.AddStructured(LogType.Damaged, oldState == MobState.Alive ? LogImpact.Low : LogImpact.Medium, $"{target:user} state changed from {oldState} to {newState}");
