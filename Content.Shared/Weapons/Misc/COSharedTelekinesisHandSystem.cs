...
public abstract partial class COSharedTelekinesisHandSystem : EntitySystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<COTelekinesisHandComponent, ActivateInWorldEvent>(OnTetherActivate);
        SubscribeLocalEvent<COTelekinesisHandComponent, AfterInteractEvent>(OnTetherRanged);
        SubscribeAllEvent<CORequestTetherMoveEvent>(OnTetherMove);

        SubscribeLocalEvent<COTelekinesisTetherComponent, BuckleAttemptEvent>(OnTetheredBuckleAttempt);
        SubscribeLocalEvent<COTelekinesisTetherComponent, UpdateCanMoveEvent>(OnTetheredUpdateCanMove);
        SubscribeLocalEvent<COTelekinesisTetherComponent, EntGotInsertedIntoContainerMessage>(OnTetheredContainerInserted);
    }

    private void OnTetheredContainerInserted(EntityUid uid, COTelekinesisTetherComponent component, EntGotInsertedIntoContainerMessage args)
    {
        if (TryComp<COTelekinesisHandComponent>(component.Tetherer, out var tetherGun))
        {
    ...
    private void OnTetheredBuckleAttempt(EntityUid uid, COTelekinesisTetherComponent component, ref BuckleAttemptEvent args)
    {
        args.Cancelled = true;
    }

    private void OnTetheredUpdateCanMove(EntityUid uid, COTelekinesisTetherComponent component, UpdateCanMoveEvent args)
    {
        args.Cancel();
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        var tetheredQuery = EntityQueryEnumerator<COTelekinesisTetherComponent, PhysicsComponent>();
    ...
    private void OnTetherMove(CORequestTetherMoveEvent msg, EntitySessionEventArgs args)
    {
    ...
    private void OnTetherRanged(EntityUid uid, COTelekinesisHandComponent component, AfterInteractEvent args)
    {
    ...
    protected bool TryGetTetherGun(EntityUid user, [NotNullWhen(true)] out EntityUid? gunUid, [NotNullWhen(true)] out COTelekinesisHandComponent? gun)
    {
        gunUid = null;
        gun = null;
    ...
    private void OnTetherActivate(EntityUid uid, COTelekinesisHandComponent component, ActivateInWorldEvent args)
    {
        if (!args.Complex)
            return;
    ...
    public bool TryTether(EntityUid gun, EntityUid target, EntityUid? user, COTelekinesisHandComponent? component = null)
    {
    ...
    protected virtual bool CanTether(EntityUid uid, COTelekinesisHandComponent component, EntityUid target, EntityUid? user)
    {
        if (HasComp<COTelekinesisTetherComponent>(target) || !TryComp<PhysicsComponent>(target, out var physics))
            return false;
    ...
    protected virtual void StartTether(EntityUid gunUid, COTelekinesisHandComponent component, EntityUid target, EntityUid? user,
        PhysicsComponent? targetPhysics = null, TransformComponent? targetXform = null)
    {
        ...
        var tethered = EnsureComp<COTelekinesisTetherComponent>(target);
    ...
    public virtual void StopTether(EntityUid gunUid, COTelekinesisHandComponent component, bool land = true, bool transfer = false)
    {
        if (component.Tethered == null)
            return;
        ...
        if (TryComp<PhysicsComponent>(component.Tethered, out var targetPhysics))
        {
            ...
            _physics.SetAngularDamping(component.Tethered.Value, targetPhysics, Comp<COTelekinesisTetherComponent>(component.Tethered.Value).OriginalAngularDamping);
        }
        ...
        RemComp<COTelekinesisTetherComponent>(component.Tethered.Value);
    ...
    protected sealed class CORequestTetherMoveEvent : EntityEventArgs
    {
