...
public sealed partial class StatusEffectsSystem : EntitySystem
{
    ...
    )
    {
        ...
        {
            Log.Warning($"Status Effect {effectProto.Id} was given no duration when being created on entity {ToPrettyString(target)}. Effect was not applied, please ensure a valid duration before creating a new status effect.");
            return false;
        }
