...
public abstract class SharedEmitSoundSystem : EntitySystem
{
    ...
    protected void TryEmitSound(EntityUid uid, BaseEmitSoundComponent component, EntityUid? user=null, bool predict=true)
    {
        ...
        if (component.Positional)
        {
            ...
            if (predict)
                _audioSystem.PlayPredicted(component.Sound, coords, user, FunAudioParams.WithUniformPitch());
            else if (_netMan.IsServer)
                // don't predict sounds that client couldn't have played already
                _audioSystem.PlayPvs(component.Sound, coords, FunAudioParams.WithUniformPitch());
        }
        else
        {
            if (predict)
                _audioSystem.PlayPredicted(component.Sound, uid, user, FunAudioParams.WithUniformPitch());
            else if (_netMan.IsServer)
                // don't predict sounds that client couldn't have played already
    ...
    private void OnEmitSoundOnCollide(EntityUid uid, EmitSoundOnCollideComponent component, ref StartCollideEvent args)
    {
        ...
        {
            _audioSystem.PlayPvs(_audioSystem.ResolveSound(sound), uid, FunAudioParams.WithUniformPitch(AudioParams.Default.WithVolume(volume)));
        }
    }
