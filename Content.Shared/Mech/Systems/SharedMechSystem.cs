...
public abstract partial class SharedMechSystem : EntitySystem
{
    ...
    private void OnEjectPilotEvent(Entity<MechComponent> ent, ref MechEjectPilotEvent args)
    {
        ...
        var doAfterEventArgs = new DoAfterArgs(EntityManager, args.Performer, ent.Comp.EntryDelay, new MechExitEvent(), ent.Owner, target: ent.Owner)
        {
            BreakOnMove = true
        };
    ...
    {
        if (!TryComp<HandsComponent>(pilot, out _))
            return;
    ...
    private void OnDragDrop(Entity<MechComponent> ent, ref DragDropTargetEvent args)
    {
        ...
        var doAfterEventArgs = new DoAfterArgs(EntityManager, args.User, ent.Comp.EntryDelay, new MechEntryEvent(), ent.Owner, target: ent.Owner, used: args.Dragged)
        {
            BreakOnMove = true,
            NeedHand = true,
        };
    ...
    private void SetupUser(Entity<MechComponent> ent, EntityUid pilot)
    {
        var rider = EnsureComp<MechPilotComponent>(pilot);
        rider.Mech = ent.Owner;
        Dirty(pilot, rider);
        ...
        ManageVirtualItems(pilot, ent.Owner, create: true);
    }
    ...
    private void OnGetVerb(Entity<MechComponent> ent, ref GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        if (CanInsert(ent, args.User))
        {
            var enterVerb = new AlternativeVerb
            {
                ...
                Act = () =>
                {
                    var doAfterEventArgs = new DoAfterArgs(EntityManager, user, ent.Comp.EntryDelay, new MechEntryEvent(), ent.Owner, target: ent.Owner)
                    {
                        BreakOnMove = true,
                        NeedHand = true,
                    };
        ...
        else if (Vehicle.HasOperator(ent.Owner))
        {
            var ejectVerb = new AlternativeVerb
            {
                ...
                Act = () =>
                {
                    var doAfterEventArgs = new DoAfterArgs(EntityManager, user, ent.Comp.ExitDelay, new MechExitEvent(), ent.Owner, target: ent.Owner)
                    {
                        BreakOnMove = true,
                    };

                    if (user != ent.Owner && user != ent.Comp.PilotSlot.ContainedEntity)
                        _popup.PopupPredicted(Loc.GetString("mech-eject-pilot-alert-popup", ("item", ent.Owner), ("user", user)), ent.Owner, user);
    ...
    private void OnMechEntry(Entity<MechComponent> ent, ref MechEntryEvent args)
    {
        ...
        if (TryComp<MechLockComponent>(ent.Owner, out var lockComp) && lockComp.IsLocked && !_mechLock.CheckAccess(ent.Owner, performer, lockComp))
        {
            _mechLock.CheckAccessWithFeedback(ent.Owner, performer, lockComp);
            return;
        }

        if (!Vehicle.CanOperate(ent.Owner, performer))
        {
            _popup.PopupPredicted(Loc.GetString("mech-no-enter-popup", ("item", ent.Owner)), performer, performer);
            return;
        }

        if (!TryInsert(ent, pilot))
            return;
        args.Handled = true;
    ...
    private void OnInteractUsing(Entity<MechComponent> ent, ref InteractUsingEvent args)
    {
        ...
        if (_tool.HasQuality(args.Used, PryingQuality) && ent.Comp.BatterySlot.ContainedEntity != null)
        {
            ...
            {
                _popup.PopupPredicted(Loc.GetString("mech-cannot-modify-closed-popup"), args.User, args.User);
                return;
            }
        ...
        if (!Vehicle.HasOperator(ent.Owner) && TryComp<MaterialComponent>(args.Used, out _))
        {
    ...
    private void OnMechCanMoveEvent(Entity<MechComponent> ent, ref UpdateCanMoveEvent args)
    {
        ...
        var hasCharge = _powerCell.TryGetBatteryFromSlot(ent.Owner, out var battery) && _battery.GetCharge(battery.Value.AsNullable()) > 0;
        ...
        if (TryComp<MechLockComponent>(ent.Owner, out var lockComp) && lockComp.IsLocked)
        {
            var pilot = Vehicle.GetOperatorOrNull(ent.Owner);
            if (pilot.HasValue && !_mechLock.CheckAccess(ent.Owner, pilot.Value, lockComp))
            {
                args.Cancel();
                return;
            }
    ...
    private void OnMechEntrySuccessSound(Entity<MechComponent> ent, ref MechEntrySuccessSoundEvent args)
    {
        ...
        _audio.PlayEntity(args.Sound, Filter.Entities(pilot.Value), ent.Owner, false);
    }
    ...
    private void OnEquipmentSelectRequest(RequestMechEquipmentSelectEvent args, EntitySessionEventArgs session)
    {
        ...
        if (args.Equipment == null)
        {
            ...
            _popup.PopupPredicted(Loc.GetString("mech-select-none-popup"), mech, mech);
        }
        else
        {
            ...
            if (Exists(equipment) && mechComp.EquipmentContainer.ContainedEntities.Any(e => e == equipment))
            {
                ...
                _popup.PopupPredicted(Loc.GetString("mech-select-popup", ("item", equipment)), mech, mech);
            }
        }
    ...
    private void OnToolUseAttempt(Entity<MechPilotComponent> ent, ref ToolUserAttemptUseEvent args)
    {
        if (args.Target == ent.Comp.Mech)
            args.Cancelled = true;
    }

    private void OnEmpAttempt(Entity<MechComponent> ent, ref EmpAttemptEvent args)
    {
    ...
    private void OnMapInit(Entity<MechComponent> ent, ref MapInitEvent args)
    {
        ...
        foreach (var equipment in ent.Comp.StartingEquipment)
        {
            ...
            InsertEquipment(ent, equipmentEnt);
        }
        ...
        EnsureComp<MechCabinAirComponent>(ent.Owner);

        ent.Comp.Integrity = ent.Comp.MaxIntegrity;
        ent.Comp.Airtight = false;

        SetIntegrity(ent, ent.Comp.MaxIntegrity);
        _actionBlocker.UpdateCanMove(ent.Owner);
