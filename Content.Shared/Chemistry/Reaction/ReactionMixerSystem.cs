...
public sealed partial class ReactionMixerSystem : EntitySystem
{
    ...
    public bool CanMix(Entity<ReactionMixerComponent?> ent, EntityUid target)
    {
        ...
        if (!_solutionContainers.TryGetMixableSolution(target, out _, out _))
            return false;
    ...
    public bool TryMix(Entity<ReactionMixerComponent?> ent, EntityUid target)
    {
        ...
        if (!_solutionContainers.TryGetMixableSolution(target, out var solutionEnt, out _))
            return false;

        _solutionContainers.UpdateChemicals(solutionEnt.Value, true, ent.Comp);
