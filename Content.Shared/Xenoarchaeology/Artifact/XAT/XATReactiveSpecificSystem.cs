...
public sealed partial class XATReactiveSpecificSystem : BaseXATSystem<XATReactiveSpecificComponent>
{
    ...
    {
        ent.Comp.Reagent = _proto.Index(ent.Comp.Reagents.ElementAt(_random.Next(ent.Comp.Reagents.Count))); //get random reagent from hashset.
        if (ent.Comp.Reagent == null) //not a real reagent, stop here.
            return;

        if (TryComp<XenoArtifactNodeComponent>(ent.Owner, out var nodeComp))
            _xenoArch.SetNodeTip(nodeComp, Loc.GetString("xenoarch-trigger-tip-reactive-specific", ("reagent", ent.Comp.Reagent)));
    }
    ...
    {
        if (!node.Comp1.ReactionMethods.Contains(args.Method)) //are we reacting in an allowed way(s).
            return;

        if (args.ReagentQuantity.Quantity < node.Comp1.MinQuantity) //was the applied reagent of at least the minimum quantity.
            return;

        if (_proto.Index(node.Comp1.Reagent) != args.Reagent) //is the reagent real.
            return;
    ...
    private void OnExamine(Entity<XenoArtifactComponent> artifact, Entity<XATReactiveSpecificComponent, XenoArtifactNodeComponent> node, ref ExaminedEvent args)
    {
        ...
        if (index == null) //not a real reagent, stop here.
            return;
        ...
        {
            var flavor = _flavor.GetLocalizedFlavorsMessage(node.Owner, new Solution(index.ID, 100)); //get appropriate flavour message as additional help.
