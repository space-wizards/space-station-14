...
comp-kitchen-spike-kill = { CAPITALIZE(THE($user)) } { CONJUGATE-HAVE($user) } forced { THE($victim) } onto { THE($this) }, killing { OBJECT($victim)} instantly!

comp-kitchen-spike-suicide-other = { CAPITALIZE(THE($victim)) } { CONJUGATE-HAVE($victim) } thrown { REFLEXIVE($victim) } on { THE($this) }!
