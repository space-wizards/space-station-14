...
ipintel-server-ratelimited = This server uses a auditing system with external verification, but the server has reached it's maximum verification limit with the external service. Please contact the administration team of the server to inform them and for further assistance or try again later.
ipintel-unknown = This server uses a auditing system with external verification, but an error was encountered while verifying your connection. Please contact the administration team of the server to inform them and for further assistance or try again later.
ipintel-suspicious = You seem to be attempting to connect using a datacenter, proxy, VPN or otherwise suspecious connection. For administrative reasons we do not allow these connections to play. If you have a VPN or similar enabled please turn it off and attempt to reconnect or contact the administration team of the server for assistance on how to continue if you believe this is false or require to use these services to play.
