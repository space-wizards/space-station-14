...
petting-success-hamster = Вы гладите {THE($target)} по {POSS-ADJ($target)} пушистой маленькой головке.
...
petting-failure-hamster = Вы тянетесь погладить {THE($target)}, но {SUBJECT($target)} пытается укусить ваш палец и только ваши быстрые рефлексы спасают от этой почти смертельной травмы
