ent-BoxMagazinePistolCaselessRifle = коробка магазинов с .25 безгильзовыми
    .desc = Полная коробка магазинов с патронами калибра .25 безгильзовый.
    .suffix = { "" }
ent-BoxMagazinePistolCaselessRifleHighVelocity = коробка магазинов с .25 безгильзовыми (высокоскоростными)
    .desc = Полная коробка магазинов с высокоскоростными патронами калибра .25 безгильзовый.
    .suffix = { "" }
ent-BoxMagazinePistolCaselessRiflePractice = коробка магазинов с .25 безгильзовыми (учебными)
    .desc = Полная коробка магазинов с учебными патронами калибра .25 безгильзовый.
    .suffix = { "" }
ent-BoxMagazineCaselessRifleRubber = коробка магазинов с .25 безгильзовыми (резиновыми)
    .desc = Полная коробка магазинов с резиновыми патронами калибра .25 безгильзовый.
    .suffix = { "" }
ent-BoxMagazineLightRifle = коробка магазинов с .30 винтовочными
    .desc = Полная коробка магазинов с патронами калибра .30 винтовочный.
    .suffix = { "" }
ent-BoxMagazineLightRifleHighVelocity = коробка магазинов с .30 винтовочными (высокоскоростными)
    .desc = Полная коробка магазинов с высокоскоростными патронами калибра .30 винтовочный.
    .suffix = { "" }
ent-BoxMagazineLightRiflePractice = коробка магазинов с .30 винтовочными (учебными)
    .desc = Полная коробка магазинов с учебными патронами калибра .30 винтовочный.
    .suffix = { "" }
ent-BoxMagazineLightRifleRubber = коробка магазинов с .30 винтовочными (резиновыми)
    .desc = Полная коробка магазинов с резиновыми патронами калибра .30 винтовочный.
    .suffix = { "" }
ent-BoxMagazineMagnum = коробка магазинов для Ламия
    .desc = Полная коробка магазинов с патронами для пистолета Ламия.
    .suffix = { "" }
ent-BoxMagazineMagnumHighVelocity = коробка магазинов для Ламия (высокоскоростными)
    .desc = Полная коробка магазинов с высокоскоростными патронами для пистолета Ламия.
    .suffix = { "" }
ent-BoxMagazineMagnumPractice = коробка магазинов для Ламия (учебными)
    .desc = Полная коробка магазинов с учебными патронами для пистолета Ламия.
    .suffix = { "" }
ent-BoxMagazineMagnumSubMachineGun = коробка магазинов для Вектор
    .desc = Полная коробка магазинов с патронами для ПП Вектор.
    .suffix = { "" }
ent-BoxMagazineMagnumSubMachineGunHighVelocity = коробка магазинов для Вектор (высокоскоростными)
    .desc = Полная коробка магазинов с высокоскоростными патронами для ПП Вектор.
    .suffix = { "" }
ent-BoxMagazineMagnumSubMachineGunPractice = коробка магазинов для Вектор (учебными)
    .desc = Полная коробка магазинов с учебными патронами для ПП Вектор.
    .suffix = { "" }
ent-BoxMagazineMagnumSubMachineGunRubber = коробка магазинов для Вектор (резиновыми)
    .desc = Полная коробка магазинов с резиновыми патронами для ПП Вектор.
    .suffix = { "" }
ent-BoxMagazinePistolSubMachineGunTopMounted = коробка магазинов WT550 .35 авто
    .desc = Полная коробка магазинов с патронами калибра .35 авто для WT550.
    .suffix = { "" }
ent-BoxMagazinePistolCalicoTopMounted = коробка магазинов Calico .35 авто
    .desc = Полная коробка магазинов с патронами калибра .35 авто для Calico.
    .suffix = { "" }
ent-BoxMagazinePistol = коробка магазинов пистолетных .35 авто
    .desc = Полная коробка пистолетных магазинов с патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolHighVelocity = коробка магазинов пистолетных .35 авто (высокоскоростные)
    .desc = Полная коробка пистолетных магазинов с высокоскоростными патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolPractice = коробка магазинов пистолетных .35 авто (учебные)
    .desc = Полная коробка пистолетных магазинов с учебными патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolRubber = коробка магазинов пистолетных .35 авто (резиновые)
    .desc = Полная коробка пистолетных магазинов с резиновыми патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolHighCapacity = коробка магазинов автопистолетных .35 авто
    .desc = Полная коробка автопистолетных магазинов с патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolHighCapacityHighVelocity = коробка магазинов автопистолетных .35 авто (высокоскоростные)
    .desc = Полная коробка автопистолетных магазинов с высокоскоростными патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolHighCapacityPractice = коробка магазинов автопистолетных .35 авто (учебные)
    .desc = Полная коробка автопистолетных магазинов с учебными патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolHighCapacityRubber = коробка магазинов автопистолетных .35 авто (резиновые)
    .desc = Полная коробка автопистолетных магазинов с резиновыми патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolSubMachineGun = коробка магазинов для ПП .35 авто
    .desc = Полная коробка магазинов для ПП с патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolSubMachineGunHighVelocity = коробка магазинов для ПП .35 авто (высокоскоростные)
    .desc = Полная коробка магазинов для ПП с высокоскоростными патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolSubMachineGunPractice = коробка магазинов для ПП .35 авто (учебные)
    .desc = Полная коробка магазинов для ПП с учебными патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazinePistolSubMachineGunRubber = коробка магазинов для ПП .35 авто (резиновые)
    .desc = Полная коробка магазинов для ПП с резиновыми патронами калибра .35 авто.
    .suffix = { "" }
ent-BoxMagazineShotgun = коробка барабанов (.50 дробь)
    .desc = Полная коробка барабанов с патронами .50 дробь.
    .suffix = { "" }
ent-BoxMagazineShotgunBeanbag = коробка барабанов (.50 травматические)
    .desc = Полная коробка барабанов с патронами .50 травматические.
    .suffix = { "" }
ent-BoxMagazineShotgunSlug = коробка барабанов (.50 пуля)
    .desc = Полная коробка барабанов с патронами .50 пуля.
    .suffix = { "" }
ent-BoxMagazineShotgunIncendiary = коробка барабанов (.50 зажигательные)
    .desc = Полная коробка барабанов с патронами .50 зажигательные.
    .suffix = { "" }
ent-BoxAmmoProvider = { ent-BoxCardboard }
    .desc = { ent-BoxCardboard.desc }
    .suffix = { "" }
ent-BoxBeanbag = коробка ружейных патронов (травматических)
    .desc = Полная коробка травматических ружейных патронов.
    .suffix = { "" }
ent-BoxLethalshot = коробка ружейных патронов (летальных)
    .desc = Полная коробка летальных ружейных патронов.
    .suffix = { "" }
ent-BoxShotgunSlug = коробка ружейных патронов (пули)
    .desc = Полная коробка пулевых ружейных патронов
    .suffix = { "" }
ent-BoxShotgunFlare = коробка ружейных патронов (фальшфейеры)
    .desc = Полная коробка ружейных патронов-фальшфейеров.
    .suffix = { "" }
ent-BoxShotgunIncendiary = коробка ружейных патронов (зажигательные)
    .desc = Полная коробка зажигательных ружейных патронов.
    .suffix = { "" }
ent-BoxShotgunPractice = коробка ружейных патронов (учебные)
    .desc = Полная коробка учебных ружейных патронов.
    .suffix = { "" }
ent-BoxShellTranquilizer = коробка ружейных патронов (транквилизаторы)
    .desc = Полная коробка ружейных патронов-транквилизаторов.
    .suffix = { "" }
ent-BoxMagazineRifle = коробка магазинов с .20 винтовочными
    .desc = Полная коробка магазинов с патронами калибра .20 винтовочный.
    .suffix = { "" }
ent-BoxMagazineRifleHighVelocity = коробка магазинов с .20 винтовочными (высокоскоростные)
    .desc = Полная коробка магазинов с высокоскоростными патронами калибра .20 винтовочный.
    .suffix = { "" }
ent-BoxMagazineRiflePractice = коробка магазинов с .20 винтовочными (учебные)
    .desc = Полная коробка магазинов с учебными патронами калибра .20 винтовочный.
    .suffix = { "" }
ent-BoxMagazineRifleRubber = коробка магазинов с .20 винтовочными (резиновые)
    .desc = Полная коробка магазинов с резиновыми патронами калибра .20 винтовочный.
    .suffix = { "" }
