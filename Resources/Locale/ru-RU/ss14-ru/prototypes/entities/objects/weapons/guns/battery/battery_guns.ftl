ent-BatteryGunBase = { ent-BaseItem }
    .desc = { ent-BaseItem.desc }
    .suffix = { "" }
ent-BatteryGunSmall = { ent-BatteryGunBase }
    .desc = { ent-BatteryGunBase.desc }
    .suffix = { "" }
ent-BatteryGunMedium = { ent-BatteryGunBase }
    .desc = { ent-BatteryGunBase.desc }
    .suffix = { "" }
ent-BatteryGunLarge = { ent-BatteryGunBase }
    .desc = { ent-BatteryGunBase.desc }
    .suffix = { "" }
ent-LaserGun = старомодный лазерный пистолет
    .desc = Оружие, использующее свет, усиленный излучением радиации.
    .suffix = { "" }
ent-MakeshiftLaser = самодельная лазерная винтовка
    .desc = Молитесь, чтобы она не обожгла вам руки.
    .suffix = { "" }
ent-LaserPistolSvalinn = лазерный пистолет Свалинн
    .desc = Дешёвый и широко используемый лазерный пистолет.
    .suffix = { "" }
ent-PulsePistolPdw = импульсный пистолет PDW-10
    .desc = Новейший энергетический пистолет, который оперативники ГБР НТ предпочитают в качестве запасного оружия.
    .suffix = { "" }
ent-LaserRifleCog = лазерный карабин
    .desc = Предпочитается службой безопасности Nanotrasen за дешевизну и простоту использования.
    .suffix = { "" }
ent-LaserRifleSniper = лазерная винтовка Nightfall
    .desc = Лазерная винтовка большой ёмкости с прицелом.
    .suffix = { "" }
ent-PulseCarbineDeac = импульсный карабин DEAC-12
    .desc = Высокотехнологичный энергетический карабин, который предпочитают оперативники ГБР NT.
    .suffix = { "" }
ent-PulseRifleDeathSquad = импульсная винтовка Destroyer
    .desc = Оружие, которое полностью оправдывает свое название — "Уничтожитель".
    .suffix = { "" }
ent-LaserCannon = лазерная пушка
    .desc = Сверхмощное мощное лазерное оружие.
    .suffix = { "" }
ent-XrayCannon = рентгеновская пушка
    .desc = Экспериментальное оружие, использующее сконцентрированную энергию рентгеновского излучения.
    .suffix = { "" }
ent-TaserGun = электрошокер
    .desc = Электрошокер малой мощности, используемый службой безопасности для подавления целей на расстоянии.
    .suffix = { "" }
ent-AntiqueLaserGun = антикварный лазерный пистолет
    .desc = Вся отделка выполнена на высочайшем уровне. Он украшен кожей ассистентов и хромом. Предмет устрашает энергетическими всплесками.
    .suffix = { "" }
