...
cmd-cvar-help =
    ...
    Если значение предоставлено, оно проанализируется и сохранится как новое значение CVar.
...
cmd-gc-help =
    ...
    Если был предоставлен аргумент, то он проанализируется как номер поколения GC и используется GC.Collect(int).
    Используйте команду 'gfc' для проведения сборки мусора, со сжатием 'кучи больших объектов' (LOH-compacting).
cmd-gc-failed-parse = Не удалось проанализировать аргумент.
...
cmd-gc_mode-help =
    ...
    Если аргумент был пропущен, он проанализируется как GCLatencyMode и будет установлен как режим задержки GC.
