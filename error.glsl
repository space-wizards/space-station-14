...
uniform ARRAY_HIGHP float waveStrengths[10];
uniform ARRAY_HIGHP vec2 positions[10];
uniform ARRAY_HIGHP float waveSpeeds[10];
uniform ARRAY_HIGHP float downScales[10];
uniform ARRAY_HIGHP float fades[10];
uniform ARRAY_HIGHP float times[10];
...
void main()
{
 ...
 highp vec2 delta = vec2 ( st . x , st . y * ratio ) - WaveCentre ;
 highp float angle = atan ( delta . y / delta . x ) ;
 highp float sinf = sin ( angle * 40.0 ) / 80.0 ;
 highp float val = dist + sinf ;
 highp float a = 3.0 ;
 highp float cosFuns = cos ( val * 20.0 - times [ i ] * waveSpeeds [ i ] ) ;
 if ( val * 20.0 > 3.1415 / 2.0 + times [ i ] * waveSpeeds [ i ] ) {
 cosFuns = 0.0 ;
 }
 ...
 brightness += pow ( finalRes , 1.5 ) * fades [ i ] * 1.5 + bs ( sin ( angle * 40.0 ) ) * 8.0 * dist ;
 }
