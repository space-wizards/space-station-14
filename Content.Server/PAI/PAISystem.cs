...
namespace Content.Server.PAI
{
    public sealed class PAISystem : SharedPAISystem
    {
        ...
        private void OnUseInHand(EntityUid uid, PAIComponent component, UseInHandEvent args)
        {
            ...
            if (EntityManager.TryGetComponent<MindComponent>(uid, out var mind) && mind.HasMind)
            {
                _popupSystem.PopupEntity(Filter.Entities(args.User), Loc.GetString("pai-system-pai-installed"), uid);
                return;
            }
            else if (EntityManager.HasComponent<GhostTakeoverAvailableComponent>(uid))
            {
                _popupSystem.PopupEntity(Filter.Entities(args.User), Loc.GetString("pai-system-still-searching"), uid);
                return;
            }
            ...
            _popupSystem.PopupEntity(Filter.Entities(args.User), Loc.GetString("pai-system-searching"), uid);
        ...
        private void AddWipeVerb(EntityUid uid, PAIComponent pai, GetVerbsEvent<ActivationVerb> args)
        {
            ...
            if (EntityManager.TryGetComponent<MindComponent>(uid, out var mind) && mind.HasMind)
            {
                ...
                verb.Act = () => {
                    ...
                    if (EntityManager.HasComponent<MindComponent>(uid))
                    {
                        ...
                        _popupSystem.PopupEntity(Filter.Entities(args.User), Loc.GetString("pai-system-wiped-device"), uid);
                        PAITurningOff(uid);
                    }
                };
                args.Verbs.Add(verb);
            ...
            else if (EntityManager.HasComponent<GhostTakeoverAvailableComponent>(uid))
            {
                ...
                verb.Act = () => {
                    ...
                    {
                        EntityManager.RemoveComponent<GhostTakeoverAvailableComponent>(uid);
                        _popupSystem.PopupEntity(Filter.Entities(args.User), Loc.GetString("pai-system-stopped-searching"), uid);
                        PAITurningOff(uid);
                    }
                };
                args.Verbs.Add(verb);
