    ...
    public sealed class VendingMachineSystem : EntitySystem
    {
        ...
        public bool IsAuthorized(EntityUid uid, EntityUid? sender, VendingMachineComponent? vendComponent = null)
        {
            ...
            if (TryComp<AccessReaderComponent?>(vendComponent.Owner, out var accessReader))
            {
                ...
                {
                    _popupSystem.PopupEntity(Filter.Pvs(uid), Loc.GetString("vending-machine-component-try-eject-access-denied"), uid);
        ...
        public void TryEjectVendorItem(EntityUid uid, InventoryType type, string itemId, bool throwItem, VendingMachineComponent? vendComponent = null)
        {
            ...
            if (entry == null)
            {
                _popupSystem.PopupEntity(Filter.Pvs(uid), Loc.GetString("vending-machine-component-try-eject-invalid-item"), uid);
                Deny(uid, vendComponent);
                return;
            }
            ...
            if (entry.Amount <= 0)
            {
                _popupSystem.PopupEntity(Filter.Pvs(uid), Loc.GetString("vending-machine-component-try-eject-out-of-stock"), uid);
                Deny(uid, vendComponent);
                return;
            }
