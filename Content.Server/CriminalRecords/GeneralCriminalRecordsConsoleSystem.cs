...
public sealed class GeneralCriminalRecordsConsoleSystem : EntitySystem
{
        ...
        CriminalRecordArrestButtonPressed msg)
    {
        ...
        _stationRecordsSystem.TryGetRecord(station!.Value, _stationRecordsSystem.Convert(component.ActiveKey!.Value), out GeneralStationRecord? stationRecord, stationRecordsComponent);
		
		var criminalRecords =
            _stationRecordsSystem.GetRecordsOfType<GeneralCriminalRecord>(station.Value, stationRecordsComponent);

		GeneralCriminalRecord? criminalRecord = null;
        foreach (var crimRec in criminalRecords)
		{
			if (crimRec.Item2.StationRecord == stationRecord)
			{
				criminalRecord = crimRec.Item2;
				break;
			}
		}
		
        if (msg.Reason != string.Empty && msg.Reason != null && msg.Name != null && msg.Session.AttachedEntity != null && criminalRecord != null && criminalRecord.Status != null)
        {
            criminalRecord.Reason = msg.Reason;
			criminalRecord.Status = SecurityStatus.Detained;
        ...
        else if (msg.Reason == string.Empty && msg.Reason != null && msg.Name != null && msg.Session.AttachedEntity != null && criminalRecord != null && criminalRecord.Status != null)
        {
            criminalRecord.Reason = string.Empty;
			criminalRecord.Status = SecurityStatus.Detained;
        ...
        CriminalRecordWantedButtonPressed msg)
    {
        ...
        {
            criminalRecord.Reason = msg.Reason;
			criminalRecord.Status = SecurityStatus.Wanted;
        ...
        CriminalRecordReleaseButtonPressed msg)
    {
        ...
        if (msg.Reason != string.Empty && msg.Reason != null && msg.Name != null && msg.Session.AttachedEntity != null && criminalRecord != null && criminalRecord.Status != null)
        {
            ...
            _radioSystem.SendRadioMessage(uid, messages[criminalRecord.Status.Value], _prototypeManager.Index<RadioChannelPrototype>("Security"), uid);
			
			criminalRecord.Reason = msg.Reason;
			criminalRecord.Status = SecurityStatus.None;
        }
    ...
    private void UpdateUserInterface(EntityUid uid, GeneralCriminalRecordsConsoleComponent? console = null)
    {
		...
		var criminalRecords =
            _stationRecordsSystem.GetRecordsOfType<GeneralCriminalRecord>(owningStation.Value, stationRecordsComponent);
