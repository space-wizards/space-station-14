...
public sealed class CrayonSystem : SharedCrayonSystem
{
    ...
    private void OnCrayonAfterInteract(EntityUid uid, CrayonComponent component, AfterInteractEvent args)
    {
        ...
        if (component.Charges <= 0)
        {
            if (component.DeleteEmpty)
                ...
                _popup.PopupEntity(Filter.Entities(args.User), Loc.GetString("crayon-interact-not-enough-left-text"), uid);
        ...
        {
            _popup.PopupEntity(Filter.Entities(args.User), Loc.GetString("crayon-interact-invalid-location"), uid);
            args.Handled = true;
            return;
        }
    ...
    {
        _popup.PopupEntity(Filter.Entities(user), Loc.GetString("crayon-interact-used-up-text", ("owner", uid)), user);
