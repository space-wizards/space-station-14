...
public sealed class EntityEffectSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ExecuteEntityEffectEvent<ActivateArtifact>>(OnExecuteActivateArtifact);
        ...
        SubscribeLocalEvent<ExecuteEntityEffectEvent<ExtinguishReaction>>(OnExecuteExtinguishReaction);
    ...
    private bool OrganCondition(OrganType condition, Entity<MetabolizerComponent?> metabolizer)
    {
    ...
    {
        _artifact.TryActivateArtifact(args.Args.TargetEntity, logMissing: false);
    }
    ...
    private void OnExecuteAreaReactionEffect(ref ExecuteEntityEffectEvent<AreaReactionEffect> args)
    {
        if (args.Args is EntityEffectReagentArgs reagentArgs)
        {
            ...
            _audio.PlayPvs(args.Effect.Sound, reagentArgs.TargetEntity, AudioHelpers.WithVariation(0.125f));
            return;
        }
    ...
    private void OnExecuteExtinguishReaction(ref ExecuteEntityEffectEvent<ExtinguishReaction> args)
    {
        if (!TryComp(args.Args.TargetEntity, out FlammableComponent? flammable))
            return;

        _flammable.Extinguish(args.Args.TargetEntity, flammable);
        if (args.Args is EntityEffectReagentArgs reagentArgs)
        {
            _flammable.AdjustFireStacks(reagentArgs.TargetEntity, args.Effect.FireStacksAdjustment * (float) reagentArgs.Quantity, flammable);
        }
        else
        {
            _flammable.AdjustFireStacks(args.Args.TargetEntity, args.Effect.FireStacksAdjustment, flammable);
        }
    }
