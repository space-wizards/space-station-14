...
{
    public sealed class PoopSystem : EntitySystem
    {
        ...
        /// <summary>
        /// Make an entity poop, if they have a stomach.
        /// </summary>
        public void Poop(EntityUid uid, float thirstAdded = -40f, float hungerAdded = -10f)
        {
            ...
            /// pooping makes you hungrier and thirstier
            ...
            var puddle = EntityManager.SpawnEntity("PuddlePoop", Transform(uid).Coordinates);

            var puddleComp = Comp<PuddleComponent>(puddle);

            SoundSystem.Play("/Audio/Effects/Diseases/pooping.ogg", Filter.Pvs(uid), uid, AudioHelpers.WithVariation(0.2f).WithVolume(-4f));

            _popupSystem.PopupEntity(Loc.GetString("disease-poop", ("person", Identity.Entity(uid, EntityManager))), uid, Filter.Pvs(uid));
