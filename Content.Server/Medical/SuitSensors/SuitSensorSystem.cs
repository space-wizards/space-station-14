...
namespace Content.Server.Medical.SuitSensors
{
    public sealed class SuitSensorSystem : EntitySystem
    {
        ...
        public SuitSensorStatus? GetSensorState(EntityUid uid, SuitSensorComponent? sensor = null, TransformComponent? transform = null)
        {
            ...
            var status = new SuitSensorStatus(ToNetEntity(uid), userName, userJob);
            switch (sensor.Mode)
            {
                case SuitSensorMode.SensorBinary:
                ...
                case SuitSensorMode.SensorCords:
                    ...
                    status.Coordinates = ToNetCoordinates(coordinates);
                    break;
            }
        ...
        public SuitSensorStatus? PacketToSuitSensor(NetworkPayload payload)
        {
            ...
            var status = new SuitSensorStatus(ToNetEntity(suitSensorUid), name, job)
            {
                IsAlive = isAlive.Value,
                TotalDamage = totalDamage,
                Coordinates = ToNetCoordinates(cords),
            };
            return status;
        }
    }
