    ...
    public sealed class MimePowersSystem : EntitySystem
    {
        ...
        public override void Update(float frameTime)
        {
            ...
            foreach (var mime in EntityQuery<MimePowersComponent>())
            {
                ...
                _popupSystem.PopupEntity(Filter.Entities(mime.Owner), Loc.GetString("mime-ready-to-repent"), mime.Owner);
            }
        }
        ...
        private void OnSpeakAttempt(EntityUid uid, MimePowersComponent component, SpeakAttemptEvent args)
        {
            ...
            _popupSystem.PopupEntity(Filter.Entities(uid), Loc.GetString("mime-cant-speak"), uid);
            args.Cancel();
        }
        ...
        private void OnInvisibleWall(EntityUid uid, MimePowersComponent component, InvisibleWallActionEvent args)
        {
            ...
            foreach (var entity in coords.GetEntitiesInTile())
            {
                ...
                {
                    _popupSystem.PopupEntity(Filter.Entities(uid), Loc.GetString("mime-invisible-wall-failed"), uid);
                    return;
                }
            }
            _popupSystem.PopupEntity(Filter.Pvs(uid), Loc.GetString("mime-invisible-wall-popup", ("mime", uid)), uid);
        ...
        public void RetakeVow(EntityUid uid, MimePowersComponent? mimePowers = null)
        {
            ...
            {
                _popupSystem.PopupEntity(Filter.Entities(uid), Loc.GetString("mime-not-ready-repent"), uid);
                return;
            }
