...
namespace Content.Server.Construction
{
    public sealed partial class ConstructionSystem
    {
        ...
        private async Task<EntityUid?> Construct(EntityUid user, string materialContainer, ConstructionGraphPrototype graph, ConstructionGraphEdge edge, ConstructionGraphNode targetNode)
        {
            ...
            if (existed)
            {
                _popup.PopupEntity(Loc.GetString("construction-system-construct-cannot-start-another-construction"), user);
                return null;
            }
            ...
            if (failed)
            {
                _popup.PopupEntity(Loc.GetString("construction-system-construct-no-materials"), user);
                FailCleanup();
                return null;
            }
        ...
        private async void HandleStartStructureConstruction(TryStartStructureConstructionMessage ev, EntitySessionEventArgs args)
        {
            ...
            {
                _popup.PopupEntity(Loc.GetString("construction-system-inside-container"), user);
                return;
            }
            ...
            if (_beingBuilt.TryGetValue(args.SenderSession, out var set))
            {
                if (!set.Add(ev.Ack))
                {
                    _popup.PopupEntity(Loc.GetString("construction-system-already-building"), user);
                    return;
                }
            }
            else
