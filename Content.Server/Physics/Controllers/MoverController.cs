...
//using Content.Shared._Starlight.Physics;
...
//    // Starlight
//    private static readonly Gauge PlayersMoverGauge = Metrics.CreateGauge(
...
//    private int _maxMobMovers = 300; // Starlight
...
//        _cfg.OnValueChanged(StarlightCCVars.MaxMobMovers, v => _maxMobMovers = v, true);
//    }
...
//        // Starlight start
//        // A thousand movers, thats just insane.

//        // Wait, were we really iterating over all paused movers just because Aghost exists and can map?
//        var ignorePausedInputQueryEnumerator = AllEntityQuery<PhysicsIgnorePausedComponent, InputMoverComponent>(); 
//        while (ignorePausedInputQueryEnumerator.MoveNext(out var uid, out var _, out var mover))
//            InsertMover((uid, mover));

//        // have to enumerate it again to filter out exactly the mobs and not the players.
//        var playersInputQueryEnumerator = EntityQueryEnumerator<MindContainerComponent, InputMoverComponent>();
//        while (playersInputQueryEnumerator.MoveNext(out var uid, out var _, out var mover))
//            InsertMover((uid, mover));

//        PlayersMoverGauge.Set(_movers.Count);
//        var limit = _movers.Count + _maxMobMovers;
