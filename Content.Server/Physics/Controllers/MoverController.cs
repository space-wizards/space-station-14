    ...
    public sealed class MoverController : SharedMoverController
    {
        ...
        private void HandleShuttleMovement(float frameTime)
        {
            ...
            foreach (var (shuttleUid, (shuttle, pilots)) in _shuttlePilots)
            {
                ...
                if (brakeInput > 0f)
                {
                    if (body.LinearVelocity.IsLongerThan(0f))
                    {
                        ...
                        // Don't overshoot
                        if (impulse.IsLongerThan(maxVelocity))
                ...
                if (linearInput.LengthEquals(0f))
                {
                ...
                else
                {
                    ...
                    if (localVel.IsLongerThanOrEqualTo(maxVelocity) && Vector2.Dot(totalForce, localVel) > 0f)
                        finalForce = Vector2.Zero; // burn would be faster if used as such

                    if (finalForce.IsLongerThan(properAccel))
