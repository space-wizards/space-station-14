...
public record struct CommandPermissionsUnassignedError(CommandSpec Command) : IConError
{
    ...
    {
        return FormattedMessage.FromMarkupPermissive($"The command {Command.FullName()} is missing permission flags and cannot be executed.");
    }
...
public record struct NoPermissionError(CommandSpec Command) : IConError
{
    public FormattedMessage DescribeInner()
    {
        return FormattedMessage.FromMarkupPermissive($"You do not have permission to execute {Command.FullName()}");
    }
