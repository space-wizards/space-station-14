...
[UsedImplicitly]
public sealed partial class BwoinkMentorSystem : SharedBwoinkAdminSystem
{
    ...
    public override void Initialize()
    {
        ...
        _sawmill = IoCManager.Resolve<ILogManager>().GetSawmill("MENTORHELP");

        var defaultParams = new MentorHelpMessageParams(
            string.Empty,
            string.Empty,
            true,
        ...
        _maxAdditionalChars = GenerateMentorHelpMessage(defaultParams).Message.Length;
    ...
    private void NotifyAdmins(ICommonSession session, string message, PlayerStatusType statusType)
    {
        ...
        var messageParams = new MentorHelpMessageParams(
            session.Name,
            message,
            true,
            roundTime,
        ...
        if (_webhookUrl != string.Empty)
        {
            ...
            var escapedText = FormattedMessage.EscapeText(message);
            messageParams.Message = escapedText;
            var discordMessage = GenerateMentorHelpMessage(messageParams);
    ...
    private void OnClientTypingUpdated(BwoinkClientTypingUpdated msg, EntitySessionEventArgs args)
    {
        ...
        var isAdmin = _adminManager.GetAdminData(args.SenderSession)?.HasFlag(AdminFlags.Mentorhelp) ?? false;
    ...
    protected override void OnBwoinkTextMessage(BwoinkTextMessage message, EntitySessionEventArgs eventArgs)
    {
        ...
        var senderAHelpAdmin = senderAdmin?.HasFlag(AdminFlags.Mentorhelp) ?? false;
        ...
        if (senderAdmin is not null &&
            senderAdmin.Flags ==
            AdminFlags.Mentorhelp) // Mentor. Not full admin. That's why it's colored differently.
        {
            bwoinkText = $"[color=cyan]{adminPrefix}{senderSession.Name}[/color]";
        }
        else if (senderAdmin is not null && senderAdmin.HasFlag(AdminFlags.Mentorhelp))
        {
            ...
            if (!admins.Contains(session.Channel))
            {
                ...
                if (_overrideClientName != string.Empty)
                {
                    ...
                    if (senderAdmin is not null &&
                        ...
                        AdminFlags.Mentorhelp) // Mentor. Not full admin. That's why it's colored differently.
                    {
                        overrideMsgText = $"[color=cyan]{adminPrefixWebhook}{_overrideClientName}[/color]";
                    }
                    else if (senderAdmin is not null && senderAdmin.HasFlag(AdminFlags.Mentorhelp))
                    {
        ...
        if (sendsWebhook)
        {
            ...
            _messageQueues[msg.UserId].Enqueue(GenerateMentorHelpMessage(messageParams));
        }
    ...
    private static DiscordRelayedData GenerateMentorHelpMessage(MentorHelpMessageParams parameters)
    {
...
public sealed class MentorHelpMessageParams
{
    ...
    public MentorHelpMessageParams(
        string username,
        string message,
        bool isAdmin,
        string roundTime,
