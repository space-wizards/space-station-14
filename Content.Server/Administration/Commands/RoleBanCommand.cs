...
public sealed class RoleBanCommand : LocalizedCommands
{
    ...
    public override async void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        switch (args.Length)
        {
            ...
            case 4:
                ...
                {
                    shell.WriteError(Loc.GetString($"cmd-{Command}-minutes-parse", ("time", args[3]), ("help", Help)));
            ...
            case 5:
                ...
                {
                    shell.WriteLine(Loc.GetString($"cmd-{Command}-severity-parse", ("severity", args[4]), ("help", Help)));
                    return;
                }

                break;
            default:
                shell.WriteError(Loc.GetString($"cmd-{Command}-arg-count"));
        ...
        if (located == null)
        {
            shell.WriteError(Loc.GetString($"cmd-{Command}-name-parse"));
        ...
        if (_proto.HasIndex<JobPrototype>(role))
            ...
            shell.WriteError(Loc.GetString($"cmd-{Command}-job-parse", ("job", role)));
    }
    ...
    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        {
            new("0", Loc.GetString($"cmd-{Command}-hint-duration-1")),
            new("1440", Loc.GetString($"cmd-{Command}-hint-duration-2")),
            new("4320", Loc.GetString($"cmd-{Command}-hint-duration-3")),
            new("10080", Loc.GetString($"cmd-{Command}-hint-duration-4")),
            new("20160", Loc.GetString($"cmd-{Command}-hint-duration-5")),
            new("43800", Loc.GetString($"cmd-{Command}-hint-duration-6")),
        };
        ...
        return args.Length switch
        {
            ...
            3 => CompletionResult.FromHint(Loc.GetString($"cmd-{Command}-hint-3")),
            4 => CompletionResult.FromHintOptions(durOpts, Loc.GetString($"cmd-{Command}-hint-4")),
