...
public sealed class PlayTimeTrackingSystem : EntitySystem
{
    ...
    {
        _jobRequirements.RemoveDisallowedJobs(ev.Player, ev.Jobs);
    }

    private void OnIsJobAllowed(ref IsJobAllowedEvent ev)
    {
        if (!_jobRequirements.IsAllowed(ev.Player, ev.JobId, null, out _))
            ev.Cancelled = true;
    }

    private void OnGetDisallowedJobs(ref GetDisallowedJobsEvent ev)
    {
        ev.Jobs.UnionWith(_jobRequirements.GetDisallowedJobs(ev.Player));
    }
