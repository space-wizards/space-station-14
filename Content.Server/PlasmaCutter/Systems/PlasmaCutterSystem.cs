...
public sealed class PlasmaCutterSystem : EntitySystem
{
    ...
    private void TurnOff(PlasmaCutterComponent comp)
    {
        ...
        SoundSystem.Play(comp.workSound.GetSound(), Filter.Pvs(comp.Owner), comp.Owner, AudioHelpers.WithVariation(0.25f));
    ...
    private void TurnOn(PlasmaCutterComponent comp, EntityUid user)
    {
        ...
        SoundSystem.Play(comp.workSound.GetSound(), playerFilter, comp.Owner, AudioHelpers.WithVariation(0.25f));
    ...
    private async void OnAfterInteract(EntityUid uid, PlasmaCutterComponent pc, AfterInteractEvent args)
    {
        ...
        if (EntityManager.TryGetComponent<LockComponent>(args.Target.Value, out var Lock))
        {
            Lock.Locked = false;
            EntityManager.RemoveComponent<LockComponent>(args.Target.Value); //Literally destroys the lock as a tell it was broken
            args.Handled = true;
            SoundSystem.Play(pc.workSound.GetSound(), Filter.Pvs(uid, entityManager: EntityManager), pc.Owner);
            pc.CurrentAmmo--;
            return;
        }
        ...
        if (!tile.IsBlockedTurf(true)) //Delete the turf
        {
            ...
            _adminLogger.Add(LogType.PlasmaCutter, LogImpact.High, $"{ToPrettyString(args.User):user} использовал плазменный резак что-бы уничтожить строение: {tile.GridUid} строение: {snapPos} в космос");
        }
        else //Delete what the user targeted
        {
            if (args.Target is { Valid: true } target)
            {              
                _adminLogger.Add(LogType.PlasmaCutter, LogImpact.High, $"{ToPrettyString(args.User):user} использовал плазменный резак что-бы уничтожить {ToPrettyString(target):цель}");
