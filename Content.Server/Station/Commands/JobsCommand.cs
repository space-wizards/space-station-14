...
public readonly record struct JobSlotRef(string Job, EntityUid Station, StationJobsSystem Jobs, IEntityManager EntityManager)
{
    public override string ToString()
    {
        ...
        {
            return Loc.GetString("command-jobs-slot", ("station", EntityManager.ToPrettyString(Station)), ("job", Job), ("mode", "other"));
        }

        if (slot == null)
        {
            return Loc.GetString("command-jobs-slot", ("station", EntityManager.ToPrettyString(Station)), ("job", Job), ("mode", "infinite"));
        }

        return Loc.GetString("command-jobs-slot", ("station", EntityManager.ToPrettyString(Station)), ("job", Job), ("mode", "slots"), ("slots", slot));
    }
