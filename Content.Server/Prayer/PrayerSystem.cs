    ...
    private void AddPrayVerb(EntityUid uid, PrayableComponent comp, GetVerbsEvent<ActivationVerb> args)
    {
        ...
        var prayerVerb = new ActivationVerb
        {
            ...
            Act = () =>
            {
                ...
                {
                    _popupSystem.PopupEntity(Loc.GetString("prayer-popup-notify-pray-locked"), uid, actor.Session, PopupType.Large);
                    return;
                }

                _quickDialog.OpenDialog(actor.Session, Loc.GetString(comp.Verb), "Message", (string message) =>
                {
                    Pray(actor.Session, comp, message);
                });
            },
