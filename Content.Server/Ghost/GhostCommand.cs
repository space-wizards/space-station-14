...
public sealed class GhostCommand : LocalizedEntityCommands
{
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        if (!_gameTicker.PlayerGameStatuses.TryGetValue(player.UserId, out var playerStatus) || playerStatus is not PlayerGameStatus.JoinedGame)
        {
            shell.WriteLine(Loc.GetString("cmd-ghost-denied-lobby"));
            return;
        }

        if (player.AttachedEntity is { Valid: true } frozen && EntityManager.HasComponent<AdminFrozenComponent>(frozen))
        {
