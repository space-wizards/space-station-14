...
namespace Content.Server.Speech.EntitySystems
{
    ...
    public sealed class ReplacementAccentSystem : EntitySystem
    {
        ...
        public string ApplyReplacements(string message, string accent)
        {
            ...
            if (prototype.FullReplacements != null && _proto.TryIndex(_random.Pick(prototype.FullReplacements), out var replacementSet))
            {
                return prototype.FullReplacements.Length != 0 ? Loc.GetString(_random.Pick(replacementSet.Values)) : "";
            }
            ...
            foreach (var (first, second) in prototype.WordReplacements)
            {
                if (!_proto.TryIndex(first, out var originalSet) || !_proto.TryIndex(second, out var replaceSet))
                    continue;

                for (var i = 0; i < originalSet.Values.Count; i++)
                {
                    var f = _loc.GetString(originalSet.Values[i]);
                    ...
                    for (var j = Regex.Count(maskMessage, $@"(?<!\w){f}(?!\w)", RegexOptions.IgnoreCase); j > 0; j--)
                    {
                        ...
                        var replacement = _loc.GetString(replaceSet.Values[i]);
