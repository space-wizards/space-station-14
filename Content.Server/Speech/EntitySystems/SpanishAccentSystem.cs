...
namespace Content.Server.Speech.EntitySystems
{
    public sealed class SpanishAccentSystem : EntitySystem
    {
        ...
        private string InsertS(string message)
        {
            ...
            if (msg.StartsWith("s", StringComparison.InvariantCulture))
            {
                return msg.Remove(0, 1).Insert(0, "es");
            }
            else if (msg.StartsWith("S", StringComparison.InvariantCulture))
            {
                return msg.Remove(0, 1).Insert(0, "Es");
            }
        ...
        private string ReplaceQuestionMark(string message)
        {
            ...
            {
                if (s.EndsWith("?", StringComparison.InvariantCulture)) // We've got a question => add ¿ to the beginning
                {
