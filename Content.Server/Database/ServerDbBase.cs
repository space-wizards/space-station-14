...
namespace Content.Server.Database
{
    public abstract class ServerDbBase
    {
            ...
            CancellationToken cancel = default)
        {
            ...
            return new PlayerPreferences(profiles, prefs.SelectedCharacterSlot, Color.FromHex(prefs.AdminOOCColor), prefs.ConstructionFavorites);
        }
        ...
        public async Task<PlayerPreferences> InitPrefsAsync(NetUserId userId, ICharacterProfile defaultProfile)
        {
            ...
            return new PlayerPreferences(new[] { new KeyValuePair<int, ICharacterProfile>(0, defaultProfile) }, 0, Color.FromHex(prefs.AdminOOCColor), prefs.ConstructionFavorites);
        }
        ...
        public async Task SaveConstructionFavoritesAsync(NetUserId userId, IReadOnlyList<string> constructionFavorites)
        {
            await using var db = await GetDb();
            var prefs = await db.DbContext.Preference.SingleAsync(p => p.UserId == userId.UserId);
            prefs.ConstructionFavorites = constructionFavorites.ToList();
