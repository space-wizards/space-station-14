...
public sealed class DockCommand : LocalizedEntityCommands
{
    ...
    {
        if (args.Length == 1)
            return CompletionResult.FromOptions(CompletionHelper.Components<DockingComponent>(args[0], EntityManager));

        if (args.Length == 2)
            return CompletionResult.FromOptions(CompletionHelper.Components<DockingComponent>(args[1], EntityManager));
