    ...
    public sealed class GasVentPumpSystem : EntitySystem
    {
        ...
        private void UpdateState(EntityUid uid, GasVentPumpComponent vent, AppearanceComponent? appearance = null)
        {
            ...
            if (!vent.Enabled)
            {
                ...
                _appearanceSystem.SetData(appearance.Owner, VentPumpVisuals.State, VentPumpState.Off, appearance);
            }
            else if (vent.PumpDirection == VentPumpDirection.Releasing)
            {
                _appearanceSystem.SetData(appearance.Owner, VentPumpVisuals.State, VentPumpState.Out, appearance);
            }
            else if (vent.PumpDirection == VentPumpDirection.Siphoning)
            {
                _appearanceSystem.SetData(appearance.Owner, VentPumpVisuals.State, VentPumpState.In, appearance);
            }
            else if (vent.Welded)
            {
                _ambientSoundSystem.SetAmbience(uid, false);
                _appearanceSystem.SetData(appearance.Owner, VentPumpVisuals.State, VentPumpState.Welded, appearance);
            }
        }
