...
public sealed class AddMapAtmosCommand : LocalizedCommands
{
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        var mapUid = int.TryParse(args[0], out var id)
            ? mapSystem.GetMapOrInvalid(new MapId(id))
            : EntityUid.Invalid;
        if (!mapUid.IsValid())
        {
        ...
        {
            _entities.RemoveComponent<MapAtmosphereComponent>(mapUid);
        ...
        atmos.SetMapAtmosphere(mapUid, space, mix);
