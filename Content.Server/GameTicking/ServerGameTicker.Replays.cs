...
public sealed partial class ServerGameTicker
{
    ...
    private void ReplayStartRound()
    {
        try
        {
            if (!Cfg.GetCVar(CCVars.ReplayAutoRecord))
                return;
            ...
            var tempDir = Cfg.GetCVar(CCVars.ReplayAutoRecordTempDir);
            ...
            {
                var baseReplayPath = new ResPath(Cfg.GetCVar(CVars.ReplayDirectory)).ToRootedPath();
    ...
    private void ReplaysOnRecordingStopped(MappingDataNode metadata)
    {
        ...
        metadata["server_id"] = new ValueDataNode(Cfg.GetCVar(CCVars.ServerId));
        metadata["server_name"] = new ValueDataNode(ServerName);
        metadata["roundId"] = new ValueDataNode(RoundId.ToString());
    }

    private ResPath GetAutoReplayPath()
    {
        var cfgValue = Cfg.GetCVar(CCVars.ReplayAutoRecordName);
