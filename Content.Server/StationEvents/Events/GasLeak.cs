    ...
    internal sealed class GasLeak : StationEvent
    {
        ...
        public override void Update(float frameTime)
        {
            ...
            if (!_foundTile ||
                ...
                !atmosphereSystem.IsSimulatedGrid(_entityManager.GetComponent<TransformComponent>(_targetGrid).GridUid))
            {
                Running = false;
                return;
            }

            var environment = atmosphereSystem.GetTileMixture(_entityManager.GetComponent<TransformComponent>(_targetGrid).GridUid, _targetTile, true);
        ...
        private void Spark()
        {
            ...
            if (_robustRandom.NextFloat() <= SparkChance)
            {
                ...
                atmosphereSystem.HotspotExpose(_entityManager.GetComponent<TransformComponent>(_targetGrid).GridUid, _targetTile, 700f, 50f, true);
