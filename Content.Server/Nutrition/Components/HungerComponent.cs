...
namespace Content.Server.Nutrition.Components
{
    ...
    public sealed class HungerComponent : SharedHungerComponent
    {
        ...
        private void UpdateCurrentThreshold()
        {
            ...
            {
                if (_currentHungerThreshold == HungerThreshold.Dead)
                    EntitySystem.Get<AdminLogSystem>().Add(LogType.Damaged, $"{Owner} has stopped starving");
                else if (calculatedHungerThreshold == HungerThreshold.Dead)
                    EntitySystem.Get<AdminLogSystem>().Add(LogType.Damaged, $"{Owner} has started starving");
