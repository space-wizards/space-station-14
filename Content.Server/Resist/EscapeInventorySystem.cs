...
public sealed class EscapeInventorySystem : EntitySystem
{
    ...
    private void AttemptEscape(EntityUid user, EntityUid container, CanEscapeInventoryComponent component, float multiplier = 1f)
    {
        ...
        _containerSystem.TryGetOuterContainer(user, Transform(user), out var outerContainer);

        _popupSystem.PopupEntity(Loc.GetString("escape-inventory-component-start-resisting"), user, user);
        _popupSystem.PopupEntity(Loc.GetString("escape-inventory-component-start-resisting-target"), outerContainer?.Owner ?? container, outerContainer?.Owner ?? container);
    }
