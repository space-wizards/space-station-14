...
namespace Content.Server.Chemistry.EntitySystems
{
    ...
    internal sealed class VaporSystem : EntitySystem
    {
        ...
        private void Update(float frameTime, VaporComponent vapor, Solution contents)
        {
            ...
            if (vapor.ReactTimer >= ReactTime && EntityManager.GetComponent<TransformComponent>(vapor.Owner).GridUid.IsValid())
            {
                vapor.ReactTimer = 0;
                var mapGrid = _mapManager.GetGrid(EntityManager.GetComponent<TransformComponent>(entity).GridUid);
