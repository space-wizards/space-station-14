...
namespace Content.Server.PlayingCard
{
    ...
    public class PlayingCardSystem : SharedPlayingCardSystem
    {
        ...
        private void OnUiseInHand(EntityUid uid, PlayingCardComponent cardComponent, UseInHandEvent args)
        {
            // if (single)
            // flip
        ...
        /// </summary>
        public EntityUid? Split(EntityUid uid, int amount, EntityCoordinates spawnPosition, SharedPlayingCardComponent? stack = null)
        {
            ...
            if (TryComp(entity, out SharedPlayingCardComponent? stackComp))
            {
        ...
        private void OnStackInteractUsing(EntityUid uid, PlayingCardComponent cardComp, InteractUsingEvent args)
        {
            if (args.Handled)
                return;

            if (!TryComp<PlayingCardComponent>(args.Used, out var otherStack))
                return;
        ...
        public void CombineWithCards(entityUid uid)
        {

        }

        public void CombineWithDeck(entityUid uid)
        {

        }

        public void ShuffleDeck(entityUid uid, List<String> cardList)
        {
            private static Random rng = new Random();
            var shuffledcards = cardList.OrderBy(a => rng.Next()).ToList();
        }

        public void MergeCards(entityUid uid, List<String> cardsToMerge, List<String> destination, bool isDeck, PlayingCardComponent cardComp)
        {

        }

        public void PullCards(entityUid uid, int count, PlayingCardComponent cardComp)
        {
