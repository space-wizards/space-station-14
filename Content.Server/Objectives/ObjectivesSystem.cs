...
public sealed class ObjectivesSystem : SharedObjectivesSystem
{
    ...
    private void OnRoundEndText(RoundEndTextAppendEvent ev)
    {
        ...
        var summaries = new Dictionary<string, Dictionary<string, Dictionary<string, List<EntityUid>>>>();
        ...
        while (query.MoveNext(out var uid, out var gameRule))
        {
            ...
            if (!summaries.ContainsKey(agent))
                summaries[agent] = new Dictionary<string, Dictionary<string, List<EntityUid>>>();

            // next group them by agent names, for example 1 donk. co and 2 waffle co. traitors
            if (!summaries[agent].ContainsKey(info.AgentName))
                summaries[agent][info.AgentName] = new Dictionary<string, List<EntityUid>>();
...
/// </remarks>
[ByRefEvent]
public record struct ObjectivesTextGetInfoEvent(List<EntityUid> Minds, string AgentName, string? Faction = null);
