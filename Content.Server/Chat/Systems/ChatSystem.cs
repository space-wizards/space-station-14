...
public sealed partial class ChatSystem : SharedChatSystem
{
        ...
        )
    {
        ...
        if (playSound)
        {
            var audioParams = announcementSound?.Params ?? AudioParams.Default;
            audioParams = audioParams.AddVolume(-2f);
            _audio.PlayGlobal(announcementSound == null ? DefaultAnnouncementSound : _audio.ResolveSound(announcementSound), Filter.Broadcast(), true, audioParams);
        }
        ...
        Color? colorOverride = null)
    {
        ...
        if (playSound)
        {
            var audioParams = announcementSound?.Params ?? AudioParams.Default;
            audioParams = audioParams.AddVolume(-2f);
            _audio.PlayGlobal(announcementSound?.ToString() ?? DefaultAnnouncementSound, filter, true, audioParams);
        }
