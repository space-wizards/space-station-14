...
namespace Content.Server.GameObjects.Components.Rotatable
{
    ...
    public class RotatableComponent : Component
    {
        ...
        {
            if (!RotateWhileAnchored && Owner.TryGetComponent(out PhysicsComponent physics))
            {
                if (physics.Anchored)
                {
        ...
        public sealed class RotateVerb : Verb<RotatableComponent>
        {
            ...
            {
                if (!ActionBlockerSystem.CanInteract(user) || (!component.RotateWhileAnchored && component.Owner.TryGetComponent(out PhysicsComponent physics) && physics.Anchored))
                {
