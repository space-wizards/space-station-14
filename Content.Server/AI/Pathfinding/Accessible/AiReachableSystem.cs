...
namespace Content.Server.AI.Pathfinding.Accessible
{
    ...
    public sealed class AiReachableSystem : EntitySystem
    {
        ...
        public bool CanAccess(EntityUid entity, EntityUid target, float range = 0.0f)
        {
            ...
            if (!xform.GridUid.IsValid())
                return false;

            var targetTile = _mapManager.GetGrid(xform.GridUid).GetTileRef(xform.Coordinates);
        ...
        public bool CanAccess(EntityUid entity, PathfindingNode targetNode)
        {
            ...
            if (xform.GridUid != targetNode.TileRef.GridUid)
                return false;

            var entityTile = _mapManager.GetGrid(xform.GridUid).GetTileRef(xform.Coordinates);
