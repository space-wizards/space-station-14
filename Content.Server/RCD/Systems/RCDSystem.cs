...
namespace Content.Server.RCD.Systems
{
    public sealed class RCDSystem : EntitySystem
    {
        ...
        private bool IsRCDStillValid(RCDComponent rcd, AfterInteractEvent eventArgs, IMapGrid mapGrid, TileRef tile, RcdMode startingMode)
        {
            ...
            {
                _popup.PopupEntity(Filter.Entities(eventArgs.User), Loc.GetString("rcd-component-no-ammo-message"), rcd.Owner);
                return false;
            }
            ...
            switch (rcd.Mode)
            {
                ...
                case RcdMode.Floors:
                    ...
                    {
                        _popup.PopupEntity(Filter.Entities(eventArgs.User), Loc.GetString("rcd-component-cannot-build-floor-tile-not-empty-message"), rcd.Owner);
                        return false;
                    }
                ...
                case RcdMode.Deconstruct:
                    ...
                    {
                        _popup.PopupEntity(Filter.Entities(eventArgs.User), Loc.GetString("rcd-component-tile-obstructed-message"), rcd.Owner);
                        return false;
                    }
                    //They tried to decon a non-turf but it's not in the whitelist
                    if (eventArgs.Target != null && !_tagSystem.HasTag(eventArgs.Target.Value, "RCDDeconstructWhitelist"))
                    {
                        _popup.PopupEntity(Filter.Entities(eventArgs.User), Loc.GetString("rcd-component-deconstruct-target-not-on-whitelist-message"), rcd.Owner);
                        return false;
                    }
                ...
                case RcdMode.Walls:
                    if (tile.Tile.IsEmpty)
                    {
                        _popup.PopupEntity(Filter.Entities(eventArgs.User), Loc.GetString("rcd-component-cannot-build-wall-tile-not-empty-message"), rcd.Owner);
                        return false;
                    }
                ...
                case RcdMode.Airlock:
                    if (tile.Tile.IsEmpty)
                    {
                        _popup.PopupEntity(Filter.Entities(eventArgs.User), Loc.GetString("rcd-component-cannot-build-airlock-tile-not-empty-message"), rcd.Owner);
                        return false;
                    }
                    if (tile.IsBlockedTurf(true))
                    {
        ...
        private void NextMode(EntityUid uid, RCDComponent rcd, EntityUid user)
        {
            ...
            _popup.PopupEntity(Filter.Entities(user), msg, rcd.Owner);
        }
    }
}
