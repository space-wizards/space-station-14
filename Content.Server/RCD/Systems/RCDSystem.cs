...
namespace Content.Server.RCD.Systems
{
    public sealed class RCDSystem : EntitySystem
    {
        ...
        private async void OnAfterInteract(EntityUid uid, RCDComponent rcd, AfterInteractEvent args)
        {
            ...
            var gridID = clickLocationMod.GetGridEntityId(EntityManager);
            if (!gridID.IsValid())
            {
                clickLocationMod = clickLocationMod.AlignWithClosestGridTile();
                gridID = clickLocationMod.GetGridEntityId(EntityManager);
            }
            ...
            switch (rcd.Mode)
            {
                ...
                case RcdMode.Floors:
                    ...
                    _logs.Add(LogType.RCD, LogImpact.High, $"{ToPrettyString(args.User):user} used RCD to set grid: {tile.GridUid} {snapPos} to floor_steel");
                    break;
                ...
                case RcdMode.Deconstruct:
                    if (!tile.IsBlockedTurf(true)) //Delete the turf
                    {
                        ...
                        _logs.Add(LogType.RCD, LogImpact.High, $"{ToPrettyString(args.User):user} used RCD to set grid: {tile.GridUid} tile: {snapPos} to space");
                    }
                ...
                case RcdMode.Walls:
                    ...
                    _logs.Add(LogType.RCD, LogImpact.High, $"{ToPrettyString(args.User):user} used RCD to spawn {ToPrettyString(ent)} at {snapPos} on grid {mapGrid.GridEntityId}");
                    break;
                case RcdMode.Airlock:
                    ...
                    _logs.Add(LogType.RCD, LogImpact.High, $"{ToPrettyString(args.User):user} used RCD to spawn {ToPrettyString(airlock)} at {snapPos} on grid {mapGrid.GridEntityId}");
                    break;
                default:
                    args.Handled = true;
