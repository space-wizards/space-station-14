...
public sealed class TargetingConsoleSystem : SharedTargetingConsoleSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<TargetingConsoleComponent, ComponentShutdown>(OnConsoleShutdown);
        SubscribeLocalEvent<TargetingConsoleComponent, PowerChangedEvent>(OnConsolePowerChange);
        SubscribeLocalEvent<TargetingConsoleComponent, AnchorStateChangedEvent>(OnConsoleAnchorChange);
        SubscribeLocalEvent<TargetingConsoleComponent, ActivatableUIOpenAttemptEvent>(OnConsoleUIOpenAttempt);
        SubscribeLocalEvent<TargetingConsoleComponent, BoundUIClosedEvent>(OnConsoleUIClose);

        SubscribeLocalEvent<GunnerComponent, MoveEvent>(HandleGunnerMove);
    }

    private void OnConsoleShutdown(EntityUid uid, TargetingConsoleComponent component, ComponentShutdown args)
    {
        if (component.SubscribedGunner == null) return;
        RemoveGunner(component.SubscribedGunner);
    }

    private void OnConsoleAnchorChange(EntityUid uid, TargetingConsoleComponent component,
    ...
    private void OnConsolePowerChange(EntityUid uid, TargetingConsoleComponent component, PowerChangedEvent args)
    {
        switch(args.Powered)
        {
            case false:
    ...
    private void OnConsoleUIOpenAttempt(EntityUid uid, TargetingConsoleComponent component,
    ...
    private void OnConsoleUIClose(EntityUid uid, TargetingConsoleComponent component, BoundUIClosedEvent args)
    {
        if ((TargetingConsoleUiKey) args.UiKey != TargetingConsoleUiKey.Key || args.Session.AttachedEntity is not { } user)
            return;
    ...
    private bool TryGunner(EntityUid user, EntityUid console)
    {
        if (!_tags.HasTag(user, "CanUseShipGuns") ||
            !TryComp<TargetingConsoleComponent>(console, out var component) ||
    ...
    private void AddGunner(EntityUid entity, TargetingConsoleComponent component)
    {
    ...
    private void RemoveGunner(GunnerComponent component)
    {
        ...
        if (console is not TargetingConsoleComponent gunner)
            return;
