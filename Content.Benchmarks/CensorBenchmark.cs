...
public class CensorBenchmark
{
    ...
    private List<string> _blockedStrings = ["sus", "suus", "suuus", "amogus", "amoogus", "amooogus"];
    // private List<string> _blockedRegexesStrings = ["sus", "amo{1,3}gus"];

    private List<Regex> _blockedRegexes = [new Regex("su{1,3}s"), new Regex("amo{1,3}gus")];
    ...
    public void Regexes()
    {
        ...
        for (var i = 0; i < N; i++)
        {
            var searchString = _blockedRegexes[i % _blockedRegexes.Count];
