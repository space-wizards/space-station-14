            ...
            foreach (var proto in sliceableEntityProtos)
            {
                ...
                foreach (var bounty in bounties)
                {
                    ...
                    foreach (var entry in bounty.Entries)
                    {
                        ...
                        Assert.That(sliceable.Count, Is.EqualTo(1), $"{proto} counts as part of cargo bounty {bounty.ID} and slices into {sliceable.Count} slices which count for the same bounty!");
                    }
                }
