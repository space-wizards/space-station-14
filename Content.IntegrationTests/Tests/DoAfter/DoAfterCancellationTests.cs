...
public sealed class DoAfterCancellationTests : InteractionTest
{
    ...
    public async Task CancelWallConstruct()
    {
        ...
        await CancelDoAfters();
        Assert.That(ClientTarget.HasValue && CEntMan.IsClientSide(ClientTarget.Value));
    ...
    public async Task CancelRepeatedTilePry()
    {
        ...
        await Server.WaitPost(() => InteractSys.UserInteraction(Player, TargetCoords, ClientTarget));
