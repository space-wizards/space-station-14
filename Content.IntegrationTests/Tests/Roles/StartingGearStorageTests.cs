...
public sealed class StartingGearPrototypeStorageTest : GameTest
{
    ...
    public async Task TestStartingGearStorage(string startingGearProtoId)
    {
        ...
        foreach (var (slot, entProtos) in gearProto.Storage)
        {
            ...
            {
                Assert.That(_sStorageSystem.CanInsert(bag, ent, out var reason),
                    $"{nameof(StartingGearPrototype)} {gearProto.ID} could not successfully put item {SToPrettyString(ent)} into storage {bag.Id}. Reason: {reason ?? "Unknown"}");
