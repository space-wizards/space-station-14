...
public sealed class ConstructionPrototypeTest : GameTest
{
    ...
    {
        var constructionComponentName = SEntMan.ComponentFactory.CompName<ConstructionComponent>();
        var constructablePrototypes = GameDataScrounger.EntitiesWithComponent(constructionComponentName);
        ...
        using (Assert.EnterMultipleScope())
        {
            foreach (var protoKey in constructablePrototypes)
            {
                ...
                Assert.That(proto.TryComp<ConstructionComponent>(constructionComponentName, out var construction));
