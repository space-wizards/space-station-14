...
";
    ...
    public async Task TestCompleteRestockProcess()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            Assert.True(entityManager.TryGetComponent(machine, out inventoryComponent!), $"Machine has no {nameof(VendingMachineInventoryComponent)}");
            Assert.True(entityManager.TryGetComponent(machine, out empEjectComponent!), $"Machine has no {nameof(VendingMachineEmpEjectComponent)}");
            Assert.True(entityManager.TryGetComponent(machine, out ejectComponent!), $"Machine has no {nameof(VendingMachineEjectComponent)}");
            Assert.True(entityManager.TryGetComponent(machine, out dispenseOnHitComponent!), $"Machine has no {nameof(DispenseOnHitComponent)}");
            Assert.True(entityManager.TryGetComponent(machine, out brokeComponent!), $"Machine has no {nameof(BrokeComponent)}");
            ...
            Assert.That(systemMachine.TryAccessMachine(packageRight,  user, machine), Is.False, "Right package is able to restock without opened access panel");
            Assert.That(systemMachine.TryAccessMachine(packageWrong, user, machine), Is.False, "Wrong package is able to restock without opened access panel");
            ...
            Assert.That(systemMachine.TryAccessMachine(packageRight, user, machine), Is.True, "Correct package is unable to restock with access panel opened");

            // Test that the wrong package does not work.
            Assert.That(systemMachine.TryMatchPackageToMachine(packageWrong, restockWrongComponent, inventoryComponent, user, machine), Is.False, "Package with invalid canRestock is able to restock machine");

            // Test that the right package does work.
            Assert.That(systemMachine.TryMatchPackageToMachine(packageRight, restockRightComponent, inventoryComponent, user, machine), Is.True, "Package with valid canRestock is unable to restock machine");
